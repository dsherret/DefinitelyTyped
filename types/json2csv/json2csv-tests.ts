...
parser.parse([]);
const transform: Transform<ExampleObj> = new Transform<ExampleObj>({
    quote: "",
});
    ...
    fields: [
        ...
        // Supports label -> derived value
        {
            ...
            value: (row: Car, field: FieldValueCallbackInfo) => {
                if (field) {
                    return (
                        (row as any)[field.label].toLowerCase() || field.default
                    );
                }
            },
...
class ParserExt extends Parser<ExampleObj> {
    constructor() {
        ...
        s = this.processCell(
            ...
            {
                label: "test",
                value: (row: object, field: FieldValueCallbackInfo) => "string",
            },
        );
