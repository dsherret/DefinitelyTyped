...
server.get<Person>("people", { gte: 1, lt: 3 }).then(function (results) {});
typedStore.get({ gte: 1, lt: 3 }).then(function (results) {});

// With an IDBKeyRange :

server
    .get<Person>("people", IDBKeyRange.bound(1, 3, false, true))
    .then(function (results) {});
typedStore
    .get(IDBKeyRange.bound(1, 3, false, true))
    .then(function (results) {});
...
server
    .query<Person>("people", "specialProperty")
    .all()
    .execute()
    .then(function (results) {});

typedStore
    .query("specialProperty")
    .all()
    .execute()
    .then(function (results) {});
...
server
    .query<Person>("people")
    .filter(function (person: any) {
        return person.group === "hipster";
    })
    .execute()
    .then(function (results) {});
...
server
    .query<Person>("people", "firstName")
    .only("Aaron")
    .then(function (results) {});

server
    .query<Person>("people", "answer")
    .bound(30, 50)
    .then(function (results) {});
...
server
    ...
    .lowerBound("marcy")
    .modify({
        views: function (profile: any) {
            return profile.views + 1;
        },
    })
    .execute()
    .then(function () {});
