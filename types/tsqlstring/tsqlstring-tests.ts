...
const sql6 = SqlString.format("UPDATE posts SET modified = ? WHERE id = ?", [
    GETDATE,
    42,
]);

const sql7 =
    "SELECT * FROM posts WHERE title=" + SqlString.escape("Hello SQL Server");
...
const sorter2 = "date";
const sql9 =
    "SELECT * FROM posts ORDER BY " + SqlString.escapeId("posts." + sorter2);

const sorter3 = "date.2";
const sql10 =
    "SELECT * FROM posts ORDER BY " + SqlString.escapeId(sorter3, true);

const userId4 = 1;
const columns = ["username", "email"];
const sql11 = SqlString.format("SELECT ?? FROM ?? WHERE id = ?", [
    columns,
    "users",
    userId4,
]);
...
const data = {
    ...
    modified: SqlString.raw("GETDATE()"),
};
const sql13 = SqlString.format("UPDATE ?? SET ? WHERE `id` = ?", [
    "users",
    data,
    userId6,
]);
