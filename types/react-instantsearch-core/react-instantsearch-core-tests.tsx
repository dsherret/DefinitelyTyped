...
() => {
    const CoolWidget = createConnector({
        ...
        (props) => (
            <div>
                The query is {props.query}, the page is {props.page}. This is an
                error:{" "}
                {
                    // @ts-expect-error
                    props.somethingElse
...
() => {
    ...
    const TypedCoolWidgetStateless = typedCoolConnector(
        (props) => (
            <div>
                The query is {props.query}, the page is {props.page}. This is an
                error:{" "}
    ...
    const TypedCoolWidget = typedCoolConnector(
        class extends React.Component<
            ConnectorProvided<Provided> & { passThruName: string }
        > {
            render() {
          ...
          */}
                        <button
                            onClick={() => props.refine("instantsearch", 15)}
                        />
                    </div>
                );
            }
...
() => {
    ...
    interface StateResultsProps extends Partial<StateResultsProvided<MyDoc>> {
    ...
    const Stateless = connectStateResults(
        (
            // @ts-expect-error
            { searchResults, additionalProp, error },
        ) => {
            // Make sure that error is optional
            ...
            return (
                <div>
                    ...
                    {searchResults.hits.map((h) => {
                        // $ExpectType string[]
                        const words =
                            h._highlightResult.field3!.compound!.matchedWords;
                        return (
                            <span>
                                {h.field2}: {words.join(",")}
                            </span>
...
() => {
    ...
    const ConnectedCurrentRefinements =
        connectCurrentRefinements(MyCurrentRefinements);
...
() => {
    const MyClearRefinements = ({
        refine,
        items,
    }: CurrentRefinementsProvided) => (
        <button onClick={() => refine(items)}>clear all</button>
    );

    const ConnectedClearRefinements =
        connectCurrentRefinements(MyClearRefinements);
...
() => {
    ...
    const MyCurrentRefinements = connectCurrentRefinements(
        ({ refine, items, query }: CurrentRefinementsProvided) => {
            return (
                <>
                    {items.map((refinement) => {
                        /*
...
() => {
    ...
    class CustomHighlight2 extends React.Component<
        HighlightProps & { limit: number }
    > {
        render() {
...
() => {
    ...
    class SearchBox extends React.Component<Props, State> {
        ...
        render() {
            const { className, translate, loadingIndicator, submit, reset } =
                this.props;
            ...
            const button = isSearchStalled
                ? "loading"
                : isCurrentQuerySubmitted
                  ? "reset"
                  : "submit";
            ...
            return (
                <div className={className}>
                    <form
                        ...
                        className={`${className}-${
                            isSearchStalled ? "form--stalledSearch" : "form"
                        }`}
                        action=""
                        role="search"
                    >
                        ...
                        <span
                            className={`${className}-loadingIndicator`}
