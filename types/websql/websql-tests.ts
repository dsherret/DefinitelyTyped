                ...
                function (res) {
                    writeRecords(
                        ...
                        [
                            {
                                name: tableOne.name,
                                data: createMockRecord(tableOne.columns),
                            },
                            {
                                name: tableOne.name,
                                data: createMockRecord(tableOne.columns),
                            ...
                            {
                                name: tableTwo.name,
                                data: createMockRecord(tableTwo.columns),
                            },
                            {
                                name: tableTwo.name,
                                data: createMockRecord(tableTwo.columns),
    ...
    function openDatabase(
        ...
        try {
            if (!window.openDatabase) {
                ...
                var db: Database = window.openDatabase(
                    name,
                    version,
                    displayName,
                    estimatedSize,
            ...
            log(
                "Failed to open database '" +
                    name +
                    "': " +
                    (JSON && JSON.stringify ? JSON.stringify(ex) : ex),
            );
        }
    }
            ...
            switch (type) {
                ...
                case "s":
                    rec[col] = String.fromCharCode.apply(
                        String,
                        new Array(Math.round(Math.random() * 21))
                            .join(".")
                            .split("")
                            .map((k) => 65 + Math.round(Math.random() * 25)),
                    );
                    break;
                default:
                    rec[col] = null;
    ...
    function writeRecord(
        ...
        var sqlStatement = {
            sql:
                ...
                tableName +
                " values(" +
                new Array(propCount + 1).join("?").split("").join(",") +
                ")",
            args: args,
        };
    ...
    function execSqlStatements<T, U>(
        ...
        function execCommand(
            ...
            xact.executeSql(
                sql,
                args || [],
                function (xact: SQLTransaction, rs: SQLResultSet): void {
                    results.push(rs);
                    resI++;
                    if (resI === sqlCount) {
                        callback(results);
