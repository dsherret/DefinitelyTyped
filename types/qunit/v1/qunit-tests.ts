...
test("test Array 1", function () {
    equal(
        [1, 2, 3],
        ["test", "foo"],
        "String '1' and number 1 have the same value",
    );
});
...
test("Primitive types and constants", function () {
    ...
    equal(QUnit.equiv("", new SafeString()), true, "primitives vs. objects");
    equal(
        QUnit.equiv(new SafeString("My String"), "My String"),
        true,
        "primitives vs. objects",
    );
    equal(
        QUnit.equiv("My String", new SafeString("My String")),
        true,
        "primitives vs. objects",
    );
    equal(
        QUnit.equiv("Bad String", new SafeString("My String")),
        false,
        "primitives vs. objects",
    );
    equal(
        QUnit.equiv(new SafeString("Bad String"), "My String"),
        false,
        "primitives vs. objects",
    );

    equal(
        QUnit.equiv(false, new SafeBoolean()),
        true,
        "primitives vs. objects",
    );
    equal(
        QUnit.equiv(new SafeBoolean(), false),
        true,
        "primitives vs. objects",
    );
    equal(
        QUnit.equiv(true, new SafeBoolean(true)),
        true,
        "primitives vs. objects",
    );
    equal(
        QUnit.equiv(new SafeBoolean(true), true),
        true,
        "primitives vs. objects",
    );
    equal(
        QUnit.equiv(true, new SafeBoolean(1)),
        true,
        "primitives vs. objects",
    );
    equal(
        QUnit.equiv(false, new SafeBoolean(false)),
        true,
        "primitives vs. objects",
    );
    equal(
        QUnit.equiv(new SafeBoolean(false), false),
        true,
        "primitives vs. objects",
    );
    equal(
        QUnit.equiv(false, new SafeBoolean(0)),
        true,
        "primitives vs. objects",
    );
    equal(
        QUnit.equiv(true, new SafeBoolean(false)),
        false,
        "primitives vs. objects",
    );
    equal(
        QUnit.equiv(new SafeBoolean(false), true),
        false,
        "primitives vs. objects",
    );

    equal(
        QUnit.equiv(new SafeObject(), {}),
        true,
        "object literal vs. instantiation",
    );
    equal(
        QUnit.equiv({}, new SafeObject()),
        true,
    ...
    equal(
        QUnit.equiv(new SafeObject(), { a: 1 }),
        false,
        "object literal vs. instantiation",
    );
    equal(
        QUnit.equiv({ a: 1 }, new SafeObject()),
        false,
        "object literal vs. instantiation",
    );
    equal(
        QUnit.equiv({ a: undefined }, new SafeObject()),
        false,
        "object literal vs. instantiation",
    );
    equal(
        QUnit.equiv(new SafeObject(), { a: undefined }),
        false,
        "object literal vs. instantiation",
    );
});
...
test("Instances", function () {
    ...
    equal(
        QUnit.equiv(a1, b1),
        false,
        "Same properties but different constructor",
    ); // failed
...
test("object with references to self wont loop", function () {
    ...
    equal(
        QUnit.equiv(circularA, circularB),
        true,
        "Should not repeat test on object (ambigous test)",
    );
    ...
    equal(
        QUnit.equiv(circularA, circularB),
        false,
        "Should not repeat test on object (unambigous test)",
    );
});
...
test("array with references to self wont loop", function () {
    ...
    equal(
        QUnit.equiv(circularA, circularB),
        true,
        "Should not repeat test on array (ambigous test)",
    );
    ...
    equal(
        QUnit.equiv(circularA, circularB),
        false,
        "Should not repeat test on array (unambigous test)",
    );
});
...
test("mixed object/array with references to self wont loop", function () {
    ...
    equal(
        QUnit.equiv(circularA, circularB),
        true,
        "Should not repeat test on object/array (ambigous test)",
    );
    ...
    equal(
        QUnit.equiv(circularA, circularB),
        false,
        "Should not repeat test on object/array (unambigous test)",
    );
});
...
QUnit.test("test2", 11, function () {
    ...
    ok(
        typeof testDoneContext.duration === "number",
        "testDone context: duration",
    );
...
QUnit.log(function (details: any) {
    if (!details.result) {
        var output =
            "FAILED: " + (details.message ? details.message + ", " : "");
        if (details.actual) {
            output +=
...
QUnit.test(
    "<script id='qunit-unescaped-test'>'test';</script>",
    1,
    function () {
        ok(
            true,
            "<script id='qunit-unescaped-asassertionsert'>'assertion';</script>",
        );
    },
);
...
QUnit.module("Date test", {
    setup: function () {
        ...
        var Date = function () {
            ok(
                false,
                "QUnit should internally be independant from Date-related manipulation and testing",
            );
            return new orgDate();
        };
    },
...
test("jsDump output", function () {
    ...
    equal(
        QUnit.jsDump.parse({ top: 5, left: 0 }),
        '{\n  "left": 0,\n  "top": 5\n}',
    );
    if (
        typeof document !== "undefined" &&
        document.getElementById("qunit-header")
    ) {
        equal(
            QUnit.jsDump.parse(document.getElementById("qunit-header")),
            '<h1 id="qunit-header"></h1>',
        );
        equal(
            QUnit.jsDump.parse(document.getElementsByTagName("h1")),
            '[\n  <h1 id="qunit-header"></h1>\n]',
        );
    }
});
...
if (typeof document !== "undefined") {
    ...
    test("running test name displayed", function () {
        ...
        ok(
            /running test name displayed/.test(displaying.innerHTML),
            "Expect test name to be found in displayed text",
        );
        ok(
            /fixture/.test(displaying.innerHTML),
    ...
    (function () {
        ...
        QUnit.test("basics", 2, function () {
            ...
            ok(
                /(^| )runtime( |$)/.test(runtime.className),
                "Runtime element exists",
            );
        ...
        QUnit.test("values", 2, function () {
            ...
            ok(
                parseInt(basics.lastChild.previousSibling.innerHTML, 10) < 50,
                "Fast runtime for trivial test",
            );
            ok(
                parseInt(slow.lastChild.previousSibling.innerHTML, 10) > 250,
...
test("check jsDump recursion", function () {
    ...
    ok(
        new RegExp("recursion\\(-10\\)").test(circdump),
...
test("Circular reference with arrays", function () {
    ...
    equal(
        arr,
        arr[0],
        "no endless stack when trying to dump arrays with circular ref",
    );
...
function testAfterDone() {
    ...
    function secondAfterDoneTest() {
        ...
        test("count previous two test's assertions", function () {
            ...
            equal(
                tests[0].firstChild.lastChild.getElementsByTagName("b")[1]
                    .innerHTML,
                "99",
            );
            equal(
                tests[1].firstChild.lastChild.getElementsByTagName("b")[1]
                    .innerHTML,
                "99",
            );
        });
