...
app.use(
    serverTimingMiddleware({
        sendHeaders: process.env.NODE_ENV !== "production",
    }),
);
...
app.use((req, res, next) => {
    ...
    req.serverTiming.addHook("substractDataTimeFromRenderTime", (metrics) => {
        ...
        if (updated.data && updated.render) {
            const renderDuration = req.serverTiming.calculateDurationSmart(
                updated.render,
            );
            const dataDuration = req.serverTiming.calculateDurationSmart(
                updated.data,
            );
