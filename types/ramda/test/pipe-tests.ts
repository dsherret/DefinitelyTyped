...
() => {
    ...
    const capitalize = (str: string) =>
        R.pipe(R.split(""), R.adjust(0, R.toUpper), R.join(""))(str);
...
() => {
    ...
    const failedFetch = (id: string): Promise<Person> =>
    ...
    const recoverFromFailureByAlternative: (id: string) => Promise<Person> =
        R.pipe(
            failedFetch,
            R.otherwise(useDefault),
