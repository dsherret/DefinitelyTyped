...
var tempFS: FileSystemSync = window.requestFileSystemSync(
    window.TEMPORARY,
    1024 * 1024,
);
...
var fsSync: FileSystemSync = window.requestFileSystemSync(
    window.TEMPORARY,
    1024 * 1024,
);
// Get the data directory, creating it if it doesn't exist.
var dataDir: DirectoryEntrySync = fsSync.root.getDirectory("data", {
    create: true,
});

// Create the lock file, if and only if it doesn't exist.
try {
    var lockFile: FileEntrySync = dataDir.getFile("lockfile.txt", {
        create: true,
        exclusive: true,
    });
} catch (ex) {
