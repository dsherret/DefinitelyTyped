...
const f = async () => {
    ...
    console.log(
        `current message count in channel ${channel} is ${messageCount1} messages`,
    );
    ...
    console.log(
        `current message count in channel ${channel} is ${messageCount2} messages`,
    );
    ...
    console.log(
        `reading messages from channel ${channel}, found ${messages2.length} messages`,
    );
...
const redisCluster = new IORedis.Cluster(
    [{ host: process.env.redisEndpoint, port: 1234 }],
    {
        redisOptions: {
            data: {
                user_next: "3",
