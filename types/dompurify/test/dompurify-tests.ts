...
const dirty =
    '<script>alert("hi")</script><p>Totally safe<p><p onerror="blowUp()">Totally not safe</p>';
...
DOMPurify.addHook(
    hookName,
    (currentNode: Element, event: DOMPurify.HookEvent) => {},
);

// test the 'SanitizeElementHookEvent' type is publicly accessible.
DOMPurify.addHook(
    "uponSanitizeElement",
    (currentNode: Element, event: DOMPurify.SanitizeElementHookEvent) => {
        if (
            currentNode.nodeName &&
            currentNode.nodeName.match(/^\w+-\w+$/) &&
            !event.allowedTags[event.tagName]
        ) {
...
DOMPurify.addHook(
    ...
    (currentNode: Element, event: DOMPurify.SanitizeAttributeHookEvent) => {
        if (
            event.attrName &&
            event.attrName.match(/^\w+-\w+$/) &&
            !event.allowedAttributes[event.attrName]
        ) {
