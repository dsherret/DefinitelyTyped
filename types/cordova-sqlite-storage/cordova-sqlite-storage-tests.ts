...
function openingDatabase() {
    ...
    db = window.sqlitePlugin.openDatabase(
        { name: "my.db", iosDatabaseLocation: "Library" },
        successcb,
        errorcb,
    );
}
...
function singleStatementTransactions(db: SQLitePlugin.Database) {
    ...
    db.executeSql(
        ...
        (resultSet) => {
            console.log(
                "got stringlength: " + resultSet.rows.item(0).stringlength,
            );
        },
        (error) => {
            console.log("SELECT error: " + error.message);
    ...
    db.sqlBatch(
        ...
        () => {
            db.executeSql("SELECT * FROM MyTable", [], (resultSet) => {
                console.log(
                    "Sample column value: " +
                        resultSet.rows.item(0).SampleColumn,
                );
            });
        },
        (error) => {
        ...
        (tx) => {
            ...
            tx.executeSql(
                ...
                (tx, resultSet) => {
                    console.log("resultSet.insertId: " + resultSet.insertId);
                    console.log(
                        "resultSet.rowsAffected: " + resultSet.rowsAffected,
                    );
                },
                (tx, error) => {
    ...
    db.readTransaction(
        (tx) => {
            tx.executeSql(
                ...
                (tx, resultSet) => {
                    console.log(
                        "resultSet.rows.item(0).uppertext: " +
                            resultSet.rows.item(0).uppertext,
                    );
                },
                (tx, error) => {
    ...
    db.transaction(
        (tx) => {
            tx.executeSql(
                ...
                (tx, res) => {
                    console.log(
                        "got stringlength: " + res.rows.item(0).stringlength,
                    );
                },
            );
        },
...
function quickInstallationTest() {
    window.sqlitePlugin.openDatabase(
        { name: "hello-world.db", location: "default" },
        (db) => {
            db.executeSql(
                "select length('tenletters') as stringlength",
                [],
