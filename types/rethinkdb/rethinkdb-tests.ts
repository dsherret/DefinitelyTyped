...
r.connect(
    ...
    function (err: Error, conn: r.Connection) {
        ...
        r.table("players")
            .filter(
                r.row.hasFields({ games_won: { championships: true } }).not(),
            )
            .run(conn, errorAndCursorCallback);
        ...
        r.table("geo")
            .getIntersecting(r.circle(center, 1000, { unit: "m" }), {
                index: "location",
            })
...
r.connect({ host: "localhost", port: 28015 }).then(function (
    conn: r.Connection,
) {
    console.log("HI", conn);
