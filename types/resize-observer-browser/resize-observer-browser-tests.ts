...
function resizeObserverCallback(entries: readonly ResizeObserverEntry[]): void {
    for (const entry of entries) {
        ...
        if (entry.borderBoxSize && entry.borderBoxSize.length > 0) {
            ...
            console.log(
                `Border Box Size is ${boxSize.blockSize}, ${boxSize.inlineSize}`,
            );
        }
        if (entry.contentBoxSize && entry.contentBoxSize.length > 0) {
            const boxSize = entry.contentBoxSize[0];
            console.log(
                `Content Box Size is ${boxSize.blockSize}, ${boxSize.inlineSize}`,
            );
        }
        if (
            ...
            console.log(
                `Device Pixel Content Box Size is ${boxSize.blockSize}, ${boxSize.inlineSize}`,
            );
        }
    }
}
