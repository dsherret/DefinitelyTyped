let defaultIceServers: RTCIceServer[] =
    window.RTCPeerConnection.defaultIceServers;
...
let ice2: RTCIceServer = {
    urls: ["stun:stunserver.org", "stun:stun.example.com"],
};
let pc: RTCPeerConnection = new RTCPeerConnection({});
let pc2: RTCPeerConnection = new RTCPeerConnection({
    iceServers: [ice1, ice2],
});
window.RTCPeerConnection.generateCertificate("sha-256").then(
    (cert: RTCCertificate) => {
...
// Offer/answer flow
pc.createOffer({ iceRestart: true }).then(
...
dtlsTransport.onerror = (ev: RTCErrorEvent) =>
    console.log(ev.error.errorDetail);
dtlsTransport.onstatechange = (ev) => console.log(ev.type);
dtlsTransport.addEventListener("error", (ev: RTCErrorEvent) =>
    console.log(ev.error.errorDetail),
);
...
iceTransport.addEventListener("ongatheringstatechange", (ev) =>
    console.log(ev.type),
);
iceTransport.addEventListener("onselectedcandidatepairchange", (ev) =>
    console.log(ev.type),
);
