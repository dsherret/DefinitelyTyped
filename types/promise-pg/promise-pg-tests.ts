...
pg.connect(conString)
    .spread(
        function (client: pg.Client, done: () => void) {
            client
...
client
    ...
    .spread(
        function (client: pg.Client, done: () => void) {
            client.query("SELECT NOW() AS 'theTime'").promise.then(
                function (result) {
...
pg.connect(conString)
    .spread(function (client: pg.Client, done: () => void) {
        var INSERT =
            "INSERT INTO users(name, t_birth, country) VALUES ($1, $2, $3)";
        ...
        trans.finally(function () {
            client
                ...
                .then(
                    ...
                    function (err) {
                        console.log(
                            "Good - Rolled back Transaction II.",
                            err.message,
                        );
                    },
                )
