...
type KeyAsStringType<K extends Key> = K extends readonly string[]
    ? K[number]
    : K;
type Rule<K extends Key, V> =
    | Key
    | ((key: K, value: V, target: Record<KeyAsStringType<K>, V>) => boolean);

type OmitMultiple<T, K extends Key> = Pick<
    T,
    Exclude<keyof T, KeyAsStringType<K>>
>;
...
declare function omit<K extends Key>(
    ...
    target: T,
) => T extends Array<infer T> ? Array<OmitMultiple<T, K>> : OmitMultiple<T, K>;
