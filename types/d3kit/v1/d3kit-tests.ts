...
describe("Skeleton", function () {
    ...
    describe("#dimension(dimension, doNotDispatch)", function () {
        ...
        it("should set width and height of the <svg> when called with an array [width, height] as the first argument", function () {
            ...
            expect([+$svg.attr("width"), +$svg.attr("height")]).to.deep.equal([
                118, 118,
            ]);
        });
        it('after setting, should dispatch "resize" event', function (done) {
...
describe.only("LayerOrganizer", function () {
    describe("new LayerOrganizer(container) will create layers as <g> by default", function () {
        ...
        describe("#create(names)", function () {
            it("should create single layer given a String", function () {
                ...
                expect(container.select("g.single-layer").size()).to.be.equal(
                    1,
                );
            });
            ...
            it("should create nested layers given a plain Object with a String inside", function () {
                ...
                expect(
                    container.select("g.d-layer g.e-layer").size(),
                ).to.be.equal(1);
            });
            ...
            it("should create nested layers given a plain Object with an Array inside", function () {
                ...
                expect(
                    container.select("g.f-layer g.g-layer").size(),
                ).to.be.equal(1);
                expect(
                    container.select("g.f-layer g.h-layer").size(),
                ).to.be.equal(1);
            });
            ...
            it("should create multiple nested layers given an array of objects", function () {
                ...
                expect(
                    container.select("g.i-layer g.x-layer").size(),
                ).to.be.equal(1);
                expect(
                    container.select("g.i-layer g.x-layer").size(),
                ).to.be.equal(1);
                expect(
                    container.select("g.k-layer g.x-layer").size(),
                ).to.be.equal(1);
                expect(
                    container.select("g.k-layer g.y-layer").size(),
                ).to.be.equal(1);
            });
            ...
            it("should create multi-level nested layers given a nested plain Object", function () {
                ...
                expect(
                    container.select("g.l-layer g.m-layer").size(),
                ).to.be.equal(1);
                expect(
                    container.select("g.l-layer g.n-layer").size(),
                ).to.be.equal(1);
                expect(
                    container.select("g.l-layer g.n-layer g.o-layer").size(),
                ).to.be.equal(1);
                expect(
                    container
                        .select("g.l-layer g.n-layer g.o-layer g.p-layer")
                        .size(),
                ).to.be.equal(1);
                expect(
                    container.select("g.l-layer g.n-layer g.q-layer").size(),
                ).to.be.equal(1);
            });
        });
    ...
    describe("new LayerOrganizer(container, tag) will create layers with the given tag instead of <g>", function () {
        ...
        describe("#create(names)", function () {
            it("should create single layer given a String", function () {
                layers.create("single");
                expect(container.select("div.single-layer").size()).to.be.equal(
                    1,
                );
            });
            ...
            it("should create nested layers given a plain Object with a String inside", function () {
                ...
                expect(
                    container.select("div.d-layer div.e-layer").size(),
                ).to.be.equal(1);
            });
            ...
            it("should create nested layers given a plain Object with an Array inside", function () {
                ...
                expect(
                    container.select("div.f-layer div.g-layer").size(),
                ).to.be.equal(1);
                expect(
                    container.select("div.f-layer div.h-layer").size(),
                ).to.be.equal(1);
            });
            ...
            it("should create multiple nested layers given an array of objects", function () {
                ...
                expect(
                    container.select("div.i-layer div.x-layer").size(),
                ).to.be.equal(1);
                expect(
                    container.select("div.i-layer div.x-layer").size(),
                ).to.be.equal(1);
                expect(
                    container.select("div.k-layer div.x-layer").size(),
                ).to.be.equal(1);
                expect(
                    container.select("div.k-layer div.y-layer").size(),
                ).to.be.equal(1);
            });
            ...
            it("should create multi-level nested layers given a nested plain Object", function () {
                ...
                expect(
                    container.select("div.l-layer div.m-layer").size(),
                ).to.be.equal(1);
                expect(
                    container.select("div.l-layer div.n-layer").size(),
                ).to.be.equal(1);
                expect(
                    container
                        .select("div.l-layer div.n-layer div.o-layer")
                        .size(),
                ).to.be.equal(1);
                expect(
                    container
                        .select(
                            "div.l-layer div.n-layer div.o-layer div.p-layer",
                        )
                        .size(),
                ).to.be.equal(1);
                expect(
                    container
                        .select("div.l-layer div.n-layer div.q-layer")
                        .size(),
                ).to.be.equal(1);
            });
        });
...
describe("Chartlet", function () {
    ...
    var enter: d3kit.ChartletEventFunction,
    ...
    var callback = function (selection?: d3.Selection<any>, done?: any) {
        return (sel: d3.Selection<any>) => {
            done();
        };
    };
    beforeEach(function (done) {
    ...
    describe("#getDispatcher()", function () {
        ...
        it("returned dispatcher should handle enter/update/exit events", function () {
            var dispatcher = chartlet.getDispatcher();
            expect(dispatcher).to.include.keys(
                ["enterDone", "updateDone", "exitDone"].concat(customEvents),
            );
        });
    });
    ...
    describe("#getPropertyValue(name, d, i)", function () {
        it("should return computed value for specified property name, d and i", function () {
            ...
            chartlet.property("baz", function (d: { a: number }, i: number) {
                return 3;
            });
            chartlet.property("qux", function (d: { a: number }, i: number) {
                return "four";
            });
    ...
    describe("#property(name, valueOrFn)", function () {
        describe("should act as a getter when called with one argument", function () {
            it("should always return a function", function () {
                ...
                chartlet.property("bar", function () {
                    return 100;
                });
    ...
    describe("#on(eventName, listener)", function () {
        it('event "enterDone" should be triggered after chartlet.enter() is completed.', function (done) {
            chartlet.on("enterDone", function () {
                return (sel: d3.Selection<any>) => {
                    done();
                };
            });
        ...
        it('event "updateDone" should be triggered after chartlet.update() is completed.', function (done) {
            chartlet.on("updateDone", function () {
                return (sel: d3.Selection<any>) => {
                    done();
                };
            });
    ...
    describe("#inheritPropertyFrom(parentChartlet, parentPropertyName, childPropertyName)", function () {
        it("it should cause a child to inherit a parent property", function () {
            var parent = ParentChartlet(function (
                parent: d3kit.Chartlet,
                child: d3kit.Chartlet,
            ) {
                child.inheritPropertyFrom(parent, "foo", "bar");
            }).property("foo", function (d: number) {
                return 2 * d;
            });
    ...
    describe("#inheritProperties(parentChartlet, parentPropertyNames, childPropertyNames)", function () {
        it("it should cause a child to inherit many parent properties", function () {
            var parent = ParentChartlet(function (
                ...
                .property("bar", function (d: number) {
                    return 3 * d;
                })
                .property("baz", function (d: number) {
                    return 4 * d;
                });
    ...
    describe("#publishEventsTo(foreignDispatcher)", function () {
        it("should map events to a foreignDispatcher", function (done) {
            var parent = new d3kit.Chartlet(callback, callback, callback, [
                "foo",
            ]);
            ...
            var child = new d3kit.Chartlet(callback, callback, callback, [
                "foo",
...
describe("#createChart", function () {
    var Chart = d3kit.factory.createChart(
        {},
        ["test"],
        function (skeleton: d3kit.Skeleton) {
            return skeleton;
        },
    );
