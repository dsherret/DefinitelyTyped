...
passport.use(
    new webauthn.Strategy(
        ...
        (id: string, userHandle: Buffer, cb: webauthn.VerifiedFunction) => {
            db.get(
                `SELECT * FROM public_key_credentials WHERE external_id = ${id}`,
                (err, row) => {
                    if (err) {
                        cb(err);
                        return;
                    ...
                    db.get(
                        `SELECT * FROM users WHERE rowid = ${row.user_id}`,
                        (err, row) => {
                            if (err) {
                                cb(err);
                                return;
        ...
        (
            ...
            id: string,
            publicKey: string,
            cb: webauthn.RegisteredFunction,
        ) => {
            db.run(
...
declare namespace db {
    function get(
        ...
        callback: (
            err: any,
            row: { user_id: string; handle: Buffer; public_key: string },
        ) => void,
    ): void;
