...
function test_basic() {
    ...
    t.push(
        toastr.warning(
            $(
                "<span>My name is Inigo Montoya. You Killed my father, prepare to die!<span>",
            ),
        ),
    );
...
function test_fromdemo() {
    ...
    $("#showtoast").click(function () {
        var shortCutFunction = $(
            ...
            ).val() as "success" | "info" | "warning" | "error",
        ...
        toastr.options = {
            ...
            positionClass:
                ($("#positionGroup input:radio:checked").val() as string) ||
                "toast-top-right",
        ...
        if ($toast.find("#okBtn").length) {
            $toast.on("click", "#okBtn", function () {
                alert(
                    "you clicked me. i was toast #" + toastIndex + ". goodbye!",
                );
                $toast.remove();
            });
        }
...
function test_displayOptions() {
    ...
    // 3rd parameter
    jQueryVal = toastr[displayMethodStr](
        stringVal,
        stringOrUndefinedVal,
        undefinedVal,
    );
    jQueryVal = toastr[displayMethodStr](
        stringVal,
