...
mk.addAccount(
    ...
    "password",
    null,
    (err: MilkCocoa.Error.AddAccount, user: milkcocoa.User) => {
        switch (err) {
            case null:
                console.log("Done.");
                break;
...
mk.login(
    "sample@test.com",
    "password",
    (err: MilkCocoa.Error.Login, user: User) => {
        if (err === MilkCocoa.Error.Login.FormatError) {
            console.log("Invalid mail address.");
        } else if (err === MilkCocoa.Error.Login.LoginError) {
            console.log(
                "Or Email that is not registered, it is not a valid password.",
            );
...
mk.dataStore("datastore")
    .child("chat")
    .child("message")
    .on("push", (data: DataStoreCallbackData) => {
