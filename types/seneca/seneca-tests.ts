...
seneca.act(
    "cmd:salestax,net:100,country:IE,category:reduced",
    function (err, result) {
...
seneca.add<SalestaxPattern, SalestaxParams>(
    { cmd: "salestax" },
...
seneca.add<ConfigPattern, ConfigParams>(
    { cmd: "config" },
    function (args, callback) {
        var config: any = {
            // any added here due to following config[] access
...
seneca.add<SalestaxPattern, SalestaxParams>(
    { cmd: "salestax" },
    function (args, callback) {
        seneca.act<ConfigRequest>(
            { cmd: "config", prop: "rate" },
            function (err, result) {
                var rate = parseFloat(result.value);
                var total = args.net * (1 + rate);
                callback(null, { total: total });
