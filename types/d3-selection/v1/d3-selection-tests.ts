...
const topSelection: d3Selection.Selection<HTMLElement, any, null, undefined> =
...
const baseTypeEl: d3Selection.Selection<
...
let body: d3Selection.Selection<HTMLBodyElement, BodyDatum, HTMLElement, any> =
    d3Selection.select<HTMLBodyElement, BodyDatum>("body");
...
const baseTypeEl2: d3Selection.Selection<
    d3Selection.BaseType,
    any,
    null,
    undefined
> = d3Selection.select(baseTypeEl.node());
// @ts-expect-error
const body2: d3Selection.Selection<HTMLElement, any, null, undefined> =
    d3Selection.select(baseTypeEl.node()); // fails as baseTypeEl.node() is of type cannot be assigned to HTMLElement

const body3: d3Selection.Selection<HTMLBodyElement, any, null, undefined> =
    d3Selection.select(body.node()!); // element types match, but datum is of type 'any' as it cannot be inferred from .node()
...
const body4: d3Selection.Selection<
    HTMLBodyElement,
    BodyDatum,
    null,
    undefined
> = d3Selection.select<HTMLBodyElement, BodyDatum>(body.node()!);

//  Explicitly cast body.node() to HTMLBodyElement to narrow selection definition.
const body5: d3Selection.Selection<
    HTMLBodyElement,
    BodyDatum,
    null,
    undefined
...
let maybeSVG1: d3Selection.Selection<
    SVGSVGElement | null,
    any,
    HTMLElement,
    undefined
...
let maybeSVG2: d3Selection.Selection<
    SVGSVGElement | null,
    any,
    null,
    undefined
...
const body7: d3Selection.Selection<
    HTMLBodyElement | null,
    any,
    HTMLElement,
    undefined
> = d3Selection.select<HTMLBodyElement | null, any>(
    // @ts-expect-error
    maybeSVG1.node(),
);
...
let emptyRootSelection: d3Selection.Selection<
    null,
    undefined,
    null,
    undefined
> = d3Selection.selectAll();
...
const baseTypeElements: d3Selection.Selection<
    d3Selection.BaseType,
    any,
    HTMLElement,
    any
> = d3Selection.selectAll("div");
...
const divElements: d3Selection.Selection<
    HTMLDivElement,
    DivDatum,
    HTMLElement,
    any
> = d3Selection.selectAll<HTMLDivElement, DivDatum>("div");

// Using selectAll(...) with node array argument and no type parameters creates selection
// with Group element of type BaseType and datum of type 'any' The parent element is of type 'null' with datum of type 'undefined'

const baseTypeElements2: d3Selection.Selection<
    d3Selection.BaseType,
    any,
    null,
    undefined
> = d3Selection.selectAll(baseTypeElements.nodes());
// fails, group elements types not of compatible for baseTypeElements
// @ts-expect-error
const divElements2: d3Selection.Selection<
    HTMLDivElement,
    any,
    null,
    undefined
...
const divElements3: d3Selection.Selection<
    HTMLDivElement,
    any,
    null,
    undefined
> = d3Selection.selectAll(divElements.nodes());
...
const divElements4: d3Selection.Selection<
    HTMLDivElement,
    DivDatum,
    null,
    undefined
> = d3Selection.selectAll<HTMLDivElement, DivDatum>(divElements.nodes());
...
const xSVGCircleElementList: NodeListOf<SVGCircleElement> =
    document.querySelectorAll("circle");
const circleSelection: d3Selection.Selection<
    SVGCircleElement,
    any,
    null,
    undefined
> = d3Selection.selectAll(xSVGCircleElementList);

// selectAll(...) accepts HTMLCollection, HTMLCollectionOf<...> argument
const documentLinks: d3Selection.Selection<
    HTMLAnchorElement | HTMLAreaElement,
    any,
    null,
    undefined
> = d3Selection.selectAll<HTMLAnchorElement | HTMLAreaElement, any>(
    document.links,
);
...
const svgEl: d3Selection.Selection<SVGSVGElement, SVGDatum, HTMLElement, any> =
    d3Selection.select<SVGSVGElement, SVGDatum>("svg");

let firstG: d3Selection.Selection<SVGGElement, SVGDatum, HTMLElement, any> =
    svgEl.select<SVGGElement>("g");
// @ts-expect-error
const firstG_2: d3Selection.Selection<
    SVGGElement,
    SVGDatum,
    SVGSVGElement,
    any
> = svgEl.select<SVGGElement>("g"); // fails, parent element of selection does not change with .select(...)
...
// test, when it is not certain, whether an element of the type to be selected exists
let maybeG: d3Selection.Selection<
    SVGGElement | null,
    SVGDatum,
    HTMLElement,
    any
...
function svgGroupSelector(
    this: SVGSVGElement,
    d: SVGDatum,
    i: number,
    groups: SVGSVGElement[] | ArrayLike<SVGSVGElement>,
): SVGGElement {
...
firstG = svgEl.select(function () {
    ...
    console.log(
        'Get <svg> Element width using "this": ',
        this.width.baseVal.value,
    ); // 'this' type is SVGSVGElement
...
firstG = svgEl.select(function (d, i, g) {
    ...
    if (g.length > 1) {
        console.log(
            "Get width of 2nd <svg> Element in group: ",
            g[1].width.baseVal.value,
        ); // type is SVGSVGElement
    }
    return this.querySelector("g")!; // this of type SVGSVGElement by type inference
});
...
let emptySubSelection: d3Selection.Selection<
    null,
    undefined,
    SVGSVGElement,
    SVGDatum
...
let elementsUnknownData: d3Selection.Selection<
    SVGGElement,
    any,
    SVGSVGElement,
    SVGDatum
> = svgEl.selectAll<SVGGElement, any>("g");
...
let gElementsOldData: d3Selection.Selection<
    SVGGElement,
    CircleDatum,
    SVGSVGElement,
    SVGDatum
> = svgEl.selectAll<SVGGElement, CircleDatum>("g");

// Using selectAll(...) sub-selection with a selector function argument.

function svgGroupSelectorAll(
    this: SVGSVGElement,
    d: SVGDatum,
    i: number,
    groups: SVGSVGElement[] | d3Selection.ArrayLike<SVGSVGElement>,
...
gElementsOldData = svgEl.selectAll<SVGGElement, CircleDatum>(
    svgGroupSelectorAll,
);

function wrongSvgGroupSelectorAll(
    this: HTMLElement,
    d: SVGDatum,
    i: number,
    groups: HTMLElement[],
...
// @ts-expect-error
gElementsOldData = svgEl.selectAll<SVGGElement, CircleDatum>(
    wrongSvgGroupSelectorAll,
); // fails, wrong this context
    ...
    function (d, i, g) {
        ...
        if (g.length > 1) {
            console.log(
                "Get width of 2nd <svg> Element in group: ",
                g[1].width.baseVal.value,
            ); // type of group is SVGSVGElement[]
        }
        return this.querySelectorAll("g");
    },
);
...
// @ts-expect-error
gElementsOldData = svgEl.selectAll<SVGGElement, CircleDatum>(function () {
    // fails, return type HTMLAnchorElement is not compatible with SVGGElement expected by selectAll-typing
...
// @ts-expect-error
gElementsUnknownData = svgEl.selectAll(function () {
    // fails, return type HTMLAnchorElement is not compatible with SVGGElement
    return this.querySelectorAll("a");
});
...
maybeG.selectAll(function (d, i, g) {
    ...
    const index: number = i;
    const group:
        | Array<SVGGElement | null>
        | d3Selection.ArrayLike<SVGGElement | null> = g;
...
gElementsOldData = svgEl.selectAll<SVGGElement, CircleDatum>(
    d3Selection.selectorAll<SVGGElement>("g"),
);
...
let filteredGElements: d3Selection.Selection<
    SVGGElement,
    CircleDatum,
    SVGSVGElement,
    SVGDatum
...
let filteredGElements2: d3Selection.Selection<
    SVGGElement,
    any,
    HTMLElement,
    any
...
filteredGElements2 = d3Selection
    .selectAll<SVGElement, any>(".any-svg-type")
    .filter<SVGGElement>("g");
// @ts-expect-error
filteredGElements2 = d3Selection.selectAll(".any-type").filter("g"); // fails without using narrowing generic on filter method

filteredGElements2 = d3Selection
    .selectAll<SVGElement, any>(".any-svg-type")
    .filter<SVGGElement>(function () {
...
let circles: d3Selection.Selection<
    SVGCircleElement,
...
circles = d3Selection
    .selectAll<SVGCircleElement, CircleDatumAlternative>("circle")
    .attr("cx", 10) // number
...
circles = circles // re-assignment test chaining return-type
    .attr("cx", function (d, i, g) {
        ...
        if (g.length > 0) {
            console.log(
                "Owner SVG Element of first group element:",
                g[0].ownerSVGElement,
            ); // group : Array<SVGCircleElement>
        }
...
divs = d3Selection
    .selectAll<HTMLDivElement, DivDatum>("div")
    .attr("contenteditable", false) // boolean
    .attr("contenteditable", () => false); // boolean return value

// classed(...) Tests

divs = divs.classed("success", true);

divs = divs.classed("zero-px-padding", function (d, i, g) {
...
divs = divs
    .style("padding", function (d, i, g) {
        ...
        const datum: DivDatum = d;
        const index: number = i;
        const group: HTMLDivElement[] | d3Selection.ArrayLike<HTMLDivElement> =
            g;
        console.log("Client Rectangle Top: ", this.getBoundingClientRect().top); // this context HTMLDivElement
        if (g.length > 0) {
            console.log("Alignment of first group element:", g[0].align); // group : Array<HTMLDivElement>
...
circles = circles.property("__hitchhikersguide__", {
    value: 42,
    survival: "towel",
}); // any

circles = circles.property("__hitchhikersguide__", function (d, i, g) {
    const that: SVGCircleElement = this;
    // @ts-expect-error
    const that2: HTMLElement = this; // fails, type mismatch
    const datum: CircleDatumAlternative = d;
    const index: number = i;
    const group: SVGCircleElement[] | d3Selection.ArrayLike<SVGCircleElement> =
        g;
    console.log("Pre-change center x-coordinate: ", this.cx.baseVal.value); // this context SVGCircleElement
    if (g.length > 0) {
        console.log(
...
body = body
    .text(function (d, i, g) {
        ...
        const group:
            | HTMLBodyElement[]
            | d3Selection.ArrayLike<HTMLBodyElement> = g;
...
body = body.html("<div> 42 </div>");

body = body.html(function (d, i, g) {
    const that: HTMLBodyElement = this;
    // @ts-expect-error
    const that2: SVGElement = this; // fails, type mismatch
    const datum: BodyDatum = d;
...
// @ts-expect-error
newBodyDatum = body
    .datum<BodyDatum>(function (d) {
        // fails, data type incompatible with return value type
        console.log("HTML5 Custom Data Attributes of body: ", this.dataset); // this typings HTMLBodyElement
        return { newFoo: "new foo" };
    })
    .datum(); // inferred type
...
d3Selection
    .select<SVGSVGElement, SVGDatum>("#svg-1")
    ...
    .classed("has-transform-property", function (d) {
        console.log(
            "Color of first data element array",
            d.length > 0 ? d[0].color : "Data array empty",
        );
...
d3Selection
    .select("#svg-1") // irrelevant typing to get contextual typing in last step of chain
...
d3Selection
    ...
    .classed("has-green-first-data-element", function (d, i, g) {
        ...
        const group:
            | d3Selection.BaseType[]
            | d3Selection.ArrayLike<d3Selection.BaseType> = g;
...
let circles2: d3Selection.Selection<
    SVGCircleElement,
    CircleDatumAlternative,
    SVGSVGElement,
    SVGDatum
...
circles2 = d3Selection
    .select<SVGSVGElement, any>("#svg2")
    .datum(dimensions)
...
let enterElements: d3Selection.Selection<
...
exitCircles
    .style("opacity", function (d, i, g) {
        ...
        const group:
            | SVGCircleElement[]
            | d3Selection.ArrayLike<SVGCircleElement> = g;
...
exitCircles2.style("opacity", function (d) {
...
td = tr
    ...
    .text(function (d, i, g) {
        ...
        const group:
            | HTMLTableDataCellElement[]
            | d3Selection.ArrayLike<HTMLTableDataCellElement> = g;
...
let newCircle: d3Selection.Selection<
    SVGCircleElement,
    BodyDatum,
    HTMLElement,
    any
...
let detachedCircle: d3Selection.Selection<
    SVGCircleElement,
    undefined,
    null,
    undefined
...
let detachedDiv: d3Selection.Selection<
    HTMLDivElement,
    undefined,
    null,
    undefined
...
let newParagraph: d3Selection.Selection<
...
newParagraph = body.insert(
    d3Selection.creator<HTMLParagraphElement>("p"),
    "p.second-paragraph",
);
newParagraph = body.insert(
    d3Selection.creator<HTMLParagraphElement>("p"),
...
let clonedParagraph: d3Selection.Selection<
    HTMLParagraphElement,
    BodyDatum,
    HTMLElement,
    any
...
// returns 'this' selection
circles = circles.each(function (d, i, g) {
    ...
    if (this.r.baseVal.value < d.r) {
        // this of type SVGCircleElement, datum of type CircleDatumAlternative
...
circles.call(
    (
        selection: d3Selection.Selection<HTMLDivElement, CircleDatum, any, any>,
    ) => {
...
circles.call(
    (
        selection: d3Selection.Selection<SVGCircleElement, DivDatum, any, any>,
    ) => {
...
let listener:
    ...
    | ((
          ...
          datum: BodyDatum,
          index: number,
          group: HTMLBodyElement[] | d3Selection.ArrayLike<HTMLBodyElement>,
      ) => void);
...
body = body.dispatch("fooEvent", function (d, i, g) {
    // re-assign for chaining test;
    const that: HTMLBodyElement = this;
    // @ts-expect-error
    const that2: SVGElement = this; // fails, type mismatch
    const datum: BodyDatum = d;
...
const resultText: string = d3Selection.customEvent(
    successEvent,
    customListener,
    body.node(),
    "Wales",
...
// @ts-expect-error
const result = d3Selection.customEvent(
    successEvent,
    customListener,
    circles.nodes()[0],
    "Wales",
); // fails, incompatible 'this' context in call
// @ts-expect-error
const resultValue: number = d3Selection.customEvent(
    successEvent,
    customListener,
    body.node(),
    "Wales",
); // fails, incompatible return types
// @ts-expect-error
d3Selection.customEvent<SVGCircleElement, any>(
    successEvent,
    customListener,
    circles.nodes()[0],
    "Wales",
); // fails, incompatible 'this' context in type parameter and call
// @ts-expect-error
d3Selection.customEvent<HTMLBodyElement, any>(
    successEvent,
    customListener,
    circles.nodes()[0],
    "Wales",
...
// @ts-expect-error
d3Selection.customEvent<HTMLBodyElement, number>(
    successEvent,
    customListener,
    body.node(),
    "Wales",
...
const svg: SVGSVGElement = d3Selection
...
const svgTextObject: d3Selection.NamespaceLocalObject | string =
    d3Selection.namespace("svg:text");
...
let selText: d3Selection.Selection<
    SVGTextElement,
    Datum,
    SVGSVGElement,
    SVGDatum
>;
let selTextAndCircle: d3Selection.Selection<
    SVGTextElement | SVGCircleElement,
    Datum,
    SVGSVGElement,
    SVGDatum
>;
...
const text = svgEl
    .selectAll<SVGTextElement, OldDatum>("text")
    .data<Datum>([{ data: "a" }]);
...
selTextAndCircle = text.join((enter) =>
    enter.append("circle").text((d) => d.data),
);
...
selText = text.join(
    "text",
    (update) =>
        r() ? undefined : update.text((d) => d.data).attr("fill", "gray"),
