...
const signature = new Uint8Array([
    77, 125, 140, 9, 158, 214, 29, 176, 0, 44, 34, 9, 111, 158, 2, 97, 66, 238,
    89, 240, 146, 171, 200, 99, 133, 231, 60, 89, 44, 156, 26, 7, 111, 198, 231,
    11, 224, 154, 151, 224, 84, 120, 183, 185, 175, 34, 165, 99, 240, 102, 21,
    251, 211, 191, 61, 224, 181, 30, 10, 108, 93, 192, 218, 43,
]);
...
crypto.subtle.importKey("pkcs8", raw_key, { name: "RANDOM" }, true, [
    "encrypt",
    "decrypt",
    "sign",
]);
crypto.subtle.importKey("spki", raw_key, { name: "RANDOM" }, true, ["wrapKey"]);
crypto.subtle.importKey("raw", raw_key, { name: "AES-GCM" }, false, [
    "encrypt",
]);
crypto.subtle
...
crypto.subtle
    ...
    .then((imported_key) => {
        crypto.subtle.sign("RSASSA-PKCS1-v1_5", imported_key, data);
    });
crypto.subtle
    .importKey(
        "jwk",
...
crypto.subtle
    .importKey("raw", keyData, { name: "AES-GCM" }, false, [
        "encrypt",
        "decrypt",
    ])
    .then((imported_key) => {
