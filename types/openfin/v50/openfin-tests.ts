function test_application() {
    ...
    application = new fin.desktop.Application(
        ...
        (successObj) => {
            console.log(
                "Application successfully created, HTTP response code:",
                successObj,
            );
            application.run();
        },
    ...
    // wrap
    application = fin.desktop.Application.wrap(
        "454C7F31-A915-4EA2-83F2-CFA655453C52",
    );
    // getWindow
    ...
    application.getGroups((allGroups) => {
        ...
        allGroups.forEach((windowGroup) => {
            console.log(
                `Group ${groupCounter} contains ${windowGroup.length} windows.`,
            );
            ++groupCounter;
        });
    });
    ...
    application.getShortcuts((config) => {
        ...
        console.log(
            "System Startup shortcut is enabled: ",
            config.systemStartup,
        );
    });
    // getTrayIconInfo
    ...
    application.setTrayIcon(
        ...
        (clickInfo) => {
            console.log(
                `The mouse has clicked at (${clickInfo.x}, ${clickInfo.y})`,
            );
        },
    );
    // setZoomLevel
...
function test_global_hot_key() {
    ...
    fin.desktop.GlobalHotkey.addEventListener(
        ...
        (event) => {
            console.log(
                ` window ${event.identity.name} has registered ${event.hotkey}`,
                event,
            );
        },
        () => {
    ...
    fin.desktop.GlobalHotkey.unregisterAll(
        ...
        (reason, err) => {
            console.log(
                "Error unregistering all hotkeys for this application",
                err,
            );
        },
    );
...
function test_inter_application_bus() {
    ...
    fin.desktop.InterApplicationBus.addSubscribeListener(
        (uuid, topic, name) => {
    ...
    const aRegisteredListener = (
        uuid: string,
        topic: string,
        name: string,
    ) => {};
    fin.desktop.InterApplicationBus.removeSubscribeListener(
        aRegisteredListener,
    );
    // removeUnsubscribeListener
    fin.desktop.InterApplicationBus.removeUnsubscribeListener(
        aRegisteredListener,
    );
    // publish
    ...
    fin.desktop.InterApplicationBus.subscribe(
        ...
        (message, uuid, name) => {
            console.log(
                `The application ${uuid} sent this message: ${message}`,
            );
        },
    );
    // unsubscribe
...
function test_system() {
    ...
    fin.desktop.System.downloadAsset(
        dirAppAsset,
        (progress) => {
            const downloadedPercent = Math.floor(
    ...
    fin.desktop.System.getAllExternalApplications((externalAppsInfoList) => {
        externalAppsInfoList.forEach((appInfo) => {
            console.log(
                `External app connected to the runtime with UUID ${appInfo.uuid}`,
            );
        });
    });
    // getAllWindows
    ...
    fin.desktop.System.getInstalledRuntimes()
        .then((runtimes) => console.log(runtimes))
    ...
    fin.desktop.System.getMonitorInfo((monitorInfo) => {
        console.log(
            "This object contains information about all monitors: ",
            monitorInfo,
        );
    });
    // getMousePosition
    fin.desktop.System.getMousePosition((mousePosition) => {
        console.log(
            `The mouse is located at left: ${mousePosition.left}, top: ${mousePosition.top}`,
        );
    });
    // getProcessList
    fin.desktop.System.getProcessList((list) => {
        list.forEach((process) => {
            console.log(
                `UUID: ${process.uuid}, Application Name: ${process.name}`,
            );
        });
    });
    // getProxySettings
    ...
    fin.desktop.System.getRuntimeInfo(
        (runtimeInfoObject) => {
            ...
            console.log(
                ...
                runtimeInfoObject["architecture"],
            );
        },
        (err) => {
    ...
    fin.desktop.System.getRvmInfo(
        (rvmInfoObject) => {
            ...
            console.log(
                "RVM has been running since:",
                rvmInfoObject["start-time"],
            );
        },
        (err) => {
    ...
    fin.desktop.System.launchExternalProcess(
        {
            ...
            certificate: {
                ...
                thumbprint:
                    "3c a5 28 19 83 05 fe 69 88 e6 8f 4b 3a af c5 c5 1b 07 80 5b",
            },
            listener(result) {
                console.log("the exit code", result.exitCode);
            },
    ...
    fin.desktop.System.registerExternalConnection(
    ...
    fin.desktop.System.Clipboard.availableFormats(
        ...
        (formats) => {
            formats.forEach((format) =>
                console.log(`The format ${format} is available to read`),
            );
        },
        (reason, err) => {
            console.log(
                `Error while reading the clipboard Message: ${err.message}, Stack: ${err.stack}`,
            );
        },
    );
    // readHtml
...
function test_window() {
    ...
    finWindow.getBounds((bounds) => {
        console.log(
            `top: ${bounds.top} left: ${bounds.left} height: ${bounds.height} width: ${bounds.width}`,
        );
    });
    // getGroups
    finWindow.getGroup((windowGroup) => {
        console.log(
            `There are a total of ${windowGroup.length} windows in this group.`,
        );
    });
    // getInfo
...
async function test_external_window() {
    // wrap
    const externalWin = await fin.ExternalWindow.wrap({
        uuid: "uuid",
        name: "name",
    });
    ...
    fin.System.getAllExternalWindows().then((exWins) =>
        exWins.forEach((exWin) => console.log(exWin.uuid)),
    );
    // addEventListener
    externalWin.addListener("some-event", (event) =>
...
async function testPlatform() {
    // ** Class Methods ** //
    // wrap
    const platform = await fin.desktop.Platform.wrap({
        uuid: "uuid",
        name: "name",
    });
    // getCurrent
    ...
    platform.reparentView(
        { uuid: "uuid", name: "view_name" },
        { uuid: "uuid", name: "target_name" },
    );
    ...
    platform.onWindowContextUpdate((newContext, oldContext) => ({
        ...oldContext,
        ...newContext,
    }));
    // quit
...
async function testView() {
    ...
    view.getOptions().then(() =>
        view.updateOptions({ autoResize: { width: true } }),
    );
    // getCurrentWindow
...
async function testLayout() {
    ...
    const wrappedLayout = await fin.desktop.Platform.Layout.wrap(
        layout.identity,
    );
    const anotherWrappedLayout = fin.desktop.Platform.Layout.wrapSync(
        layout.identity,
    );
    ...
    layout
        .getConfig()
        .then((config) => config.settings && config.settings.hasHeaders);
    layout.identity.uuid;
}

async function testFDC3() {
    const contextListener: fdc3.ContextListener = fdc3.addContextListener(
    ...
    const intentListener: fdc3.IntentListener = fdc3.addIntentListener(
        "test-intent",
        (context: fdc3.Context) => {},
    );
    ...
    const appIntents: fdc3.AppIntent[] = await fdc3.findIntentsByContext({
        type: "test-context",
    });

    const channelById: fdc3.Channel =
        await fdc3.getChannelById("test-channel-id");
    await channelById.join();

    const currentChannel: fdc3.Channel = await fdc3.getCurrentChannel();
    await currentChannel.join();

    const appChannel: fdc3.Channel = await fdc3.getOrCreateAppChannel(
        "test-app-channel-name",
    );
