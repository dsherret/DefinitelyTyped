...
new Nightmare()
    ...
    .evaluate(
        () => {
            return (document.querySelector(".url.breadcrumb") as HTMLElement)
                .innerText;
        },
        (breadcrumb: string) => {
...
new Nightmare().goto("http://www.wikipedia.org/").refresh().run(done);
...
new Nightmare()
    ...
    .evaluate(
        function () {
            return (
                document.getElementById("uploaded_file") as HTMLInputElement
            ).value;
        },
        function (value) {},
    )
...
new Nightmare().goto("http://yahoo.com").screenshot("test/test.png").run(done);
...
new Nightmare().goto("http://yahoo.com").pdf("test/test.pdf").run(done);
...
new Nightmare().goto("http://www.google.com/").wait("input").run(done);

var seconds = function () {
    var gifs = document.querySelectorAll("img");
    var split = (gifs[gifs.length - 2] as HTMLImageElement).src.split(
        ".gif",
    )[0];
    var seconds = split.split(".com/c")[1];
    return parseInt(seconds, 10);
};
new Nightmare().goto("http://onlineclock.net/").wait(seconds, 1).run(done);
...
new Nightmare().use(search("test term")).use(testTitle("test term")).run(done);
...
new Nightmare({ gotoTimeout: 10000 }).goto("https://45.56.178.93/");
