...
validateHtml({
    ...
    if (validationResults.messages.length === 0) {
        console.warn(
            `File "test" contains W3C standard violations or guidelines neglects.`,
        );
        return;
    }

    validationResults.messages.forEach(
        (violation: validateHtml.ValidationMessageObject) => {
            switch (violation.type) {
                ...
                case "info":
                    console.warn(
                        `W3C guidelines neglect: ${violation.message}`,
                    );
                    break;
            ...
            if (
                !!(violation as validateHtml.ValidationMessageLocationObject)
                    .extract
            ) {
                const v =
                    violation as validateHtml.ValidationMessageLocationObject;
                ...
                console.log(
                    `line: ${v.lastLine}, column: ${v.firstColumn}-${v.lastColumn}\n`,
                );
            }
        },
    );
