...
pinoHttp({
    autoLogging: {
        ignore: (req) => req.headers["user-agent"] === "ELB-HealthChecker/2.0",
    },
});
pinoHttp({ autoLogging: { ignorePaths: ["/health"] } });
pinoHttp({ autoLogging: { ignorePaths: [/\/health/] } });
pinoHttp({
    autoLogging: { ignorePaths: ["/health"], getPath: (req) => req.url },
});
...
pinoHttp({
    reqCustomProps: (req, res) => ({ key1: "value1", "x-key-2": "value2" }),
});
