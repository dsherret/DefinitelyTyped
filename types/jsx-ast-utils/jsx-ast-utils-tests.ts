...
declare let openingElement:
    | ESTreeJSX.JSXOpeningElement
    | ESTreeJSX.JSXOpeningFragment;
...
eventHandlersByType.composition.map(
    (propName) => propName === "onCompositionEnd",
);
...
eventHandlersByType.transition.map(
    (propName) => propName === "onTransitionEnd",
);
