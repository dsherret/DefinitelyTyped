...
request(
    ...
    (response: AllIndirectionsResponse) => {
        response.result.forEach((value) => {
            console.log(
                `${value.uid}:: ${value.name} is a ${value.resourceType}`,
            );
        });
    },
);
...
request(
    "GET",
    "http://localhost:80/api/v1/indirections/" + indirectionUid,
    (response: IndirectionByUidResponse) => {
...
request(
    ...
    (response: AllResourcesResponse<typeof resourceType>) => {
        response.result.forEach((value) => {
            console.log(
                `${value.uid}:: ${value.name} is a ${value.type} stored at ${value.path}`,
            );
        });
    },
);
...
request(
    "GET",
    "http://localhost:80/api/v1/resources/" + resourceUid,
    (response: ResourceByUidResponse) => {
        const value = response.result;
        console.log(
            `${value.uid}:: ${value.name} is a ${value.type} stored at ${value.path}`,
        );
