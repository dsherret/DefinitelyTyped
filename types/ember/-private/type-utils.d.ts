...
export type MethodParams<T, M extends MethodNamesOf<T>> = Parameters<
    MethodsOf<T>[M]
>;

export type MethodReturns<T, M extends MethodNamesOf<T>> = ReturnType<
    MethodsOf<T>[M]
>;

/** Get the return value of a method string name or a function. */
export type EmberMethodParams<
    T,
    M extends EmberMethod<T>,
> = M extends AnyMethod<T>
    ? Parameters<M>
    : M extends keyof T
      ? T[M] extends AnyMethod<T>
          ? Parameters<MethodsOf<T>[M]>
          : never
      : never;

/** Get the return value of a method string name or a function. */
export type EmberMethodReturn<
    T,
    M extends EmberMethod<T>,
> = M extends AnyMethod<T>
    ? ReturnType<M>
