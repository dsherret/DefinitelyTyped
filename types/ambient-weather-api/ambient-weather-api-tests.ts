...
async function getDevicesAndData() {
    ...
    devices.forEach(async (device) => {
        ...
        const data2: DeviceData[] = await api.deviceData(device.macAddress, {
            limit: 5,
        });
        const data3: DeviceData[] = await api.deviceData(device.macAddress, {
            endDate: "2018-01-08T18:35:00.000Z",
        });
    });
}
