...
test("event emitters bound to CLS context", function (t) {
    ...
    t.test(
        "handler registered out of context, emit out of context",
        function (t) {
            t.plan(1);
    ...
    t.test("once handler registered out of context on Readable", function (t) {
        ...
        if (Readable) {
            ...
            n.run(function () {
                ...
                t.equal(
                    typeof re._events.data,
                    "function",
                    "only the one data listener",
                );
                t.notOk(
                    re._events.data["context@outOnceReadable"],
                    "context isn't on listener",
                );
    ...
    t.test("emitter with newListener that removes handler", function (t) {
        ...
        ee.on("newListener", function handler(event: any) {
            ...
            t.notOk(
                this.listeners("newListener").length,
                "newListener was removed",
            );
    ...
    t.test("handler registered in context on Readable", function (t) {
        ...
        if (Readable) {
            ...
            t.ok(
                re._events.data["cls@contexts"]["context@outOnReadable"],
    ...
    t.test(
        "listener with parameters added but not bound to context",
        function (t) {
            t.plan(2);
    ...
    t.test(
        "listener that throws doesn't leave removeListener wrapped",
        function (t) {
            t.plan(4);
            ...
            n.run(function () {
                ...
                t.equal(
                    typeof ee.removeListener,
                    "function",
                    "removeListener is still there",
                );
    ...
    t.test(
        "emitter bound to multiple namespaces handles them correctly",
        function (t) {
            t.plan(8);
                        ...
                        process.nextTick(function () {
                            ...
                            ee.on("data", function () {
                                t.equal(
                                    ns1.get("name"),
                                    "bob",
                                    "ns1 value bound onto emitter",
                                );
                                t.equal(
                                    ns2.get("name"),
                                    "alice",
                                    "ns2 value bound onto emitter",
                                );
                            });
                        });
                    });
...
test("simple tracer built on contexts", function (t) {
    ...
    harvester.on("finished", function (transaction: Transaction) {
        ...
        t.equal(
            transaction.status,
            "ok",
            "transaction should have finished OK",
        );
    ...
    trace.runHandler(function inScope() {
        ...
        t.equal(
            tracer.get<Transaction>("transaction")!.status,
            "ok",
