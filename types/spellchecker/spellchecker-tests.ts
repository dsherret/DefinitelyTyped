...
let testWord = "kat";
let testCorpus =
    "The queeck brown fox jumped over the lazy dog, from Xynoronmalade";
let testCorpusAsync =
    "The queeck brown fox jumped over the lazy dog, from Xynoronmalade";
...
const asyncSpellCheck = async () => {
    ...
    misspellings.forEach((location) => {
        ...
        const corrections =
            Spellchecker.getCorrectionsForMisspelling(misspelled);
        if (corrections.length > 0) {
            testCorpusAsync = testCorpusAsync.replace(
                misspelled,
                corrections[0],
            );
        }
