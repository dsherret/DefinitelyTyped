...
describe("elementIdAttribute command demo", function () {
    ...
    test("async demo test", async function (browser) {
        ...
        const result = await browser.elementIdAttribute(
            webElement.getId(),
            "title",
        );
...
describe("elementIdCssProperty command demo", function () {
    ...
    test("async demo test", async function (browser) {
        const webElement = await browser.findElement("input[type=text]");
        const result = await browser.elementIdCssProperty(
            webElement.getId(),
            "background-color",
        );
        isType<string | null>(result);
...
describe("acceptAlert command demo", function () {
    before((browser) =>
        browser.url("https://nightwatchjs.org/__e2e/window/alerts.html/"),
    );
...
describe("elementIdElement command demo", function () {
    ...
    test("async demo test", async function (browser) {
        const webElement = await browser.findElement("input[type=text]");
        const result = await browser.elementIdElement(
            webElement.getId(),
            "css selector",
            "body",
        );
...
describe("executeAsyncScript command demo", function () {
    ...
    test("demo test", async function (browser) {
        ...
        const result1 = await browser.executeAsyncScript(function (
            done: () => void,
        ) {
            return "nightwatch";
        });
        ...
        const result5 = await browser.executeAsyncScript("something", [
            "something",
            5,
        ]);
        const result5Type: UnknownToTrue<typeof result5> = true;
...
describe("executeAsync command demo", function () {
    ...
    test("demo test", async function (browser) {
        ...
        const result5 = await browser.executeAsync("something", [
            "something",
            5,
        ]);
        const result5Type: UnknownToTrue<typeof result5> = true;
