...
type Result<T extends [unknown, ...unknown[]]> = Promise<
    allSettled.PromiseResultTuple<T>
>;
...
const r1: Result<[number, string]> = allSettled([
    Promise.resolve(1),
    Promise.resolve("a"),
]);
const r2: Result<[never, boolean]> = allSettled([
    Promise.reject<never>(null),
