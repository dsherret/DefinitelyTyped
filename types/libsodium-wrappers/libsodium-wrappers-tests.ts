...
sodium.ready.then(() => {
    ...
    sodium.crypto_pwhash_str_needs_rehash("password", 1, 1); // $ExpectType boolean

    // @ts-expect-error
    const wrongPasswordType = sodium.crypto_pwhash_str_needs_rehash(42, 1, 1); // $ExpectType boolean

    // @ts-expect-error
    const wrongOpsLimitType = sodium.crypto_pwhash_str_needs_rehash(
        "password",
        "wronOpsLimitType",
        1,
    ); // $ExpectType boolean
    ...
    // @ts-expect-error
    const wrongMemLimitType = sodium.crypto_pwhash_str_needs_rehash(
        "password",
        1,
        "wrongMemLimitType",
    ); // $ExpectType boolean
