...
cluster.authenticate(
    new couchbase.ClassicAuthenticator(
        { bucket: "password" },
        "username",
        "password",
    ),
...
function n1ql_a() {
    const n1qlquery = N1qlQuery.fromString(
...
function n1ql_c() {
    ...
    bucket.upsert(
        ...
        function (err, res) {
            ...
            var qs =
                'SELECT t.*, TOSTRING(META().cas) AS `_cas` FROM `travel-sample` t WHERE META().id="test-doc"';
...
function search_a() {
    let searchQuery = SearchQuery.new(
        "travel-search",
        SearchQuery.term("office"),
    );
...
function search_b() {
    const searchQuery = SearchQuery.new(
        "travel-search",
        SearchQuery.term("office"),
    );
...
function userSearch(
    ...
    var qp = couchbase.SearchQuery.conjuncts(
    ...
    if (description && description !== "*") {
        qp.and(
            couchbase.SearchQuery.disjuncts(
                couchbase.SearchQuery.matchPhrase(description).field(
                    "description",
                ),
