describe("Jasmine jQuery extension", () => {
    it("Adds jQuery matchers", () => {
        ...
        expect<JQuery>(
            $('<div><span class="some-class"></span></div>'),
        ).toContain("span.some-class");
        expect($("<span></span>").addClass("js-something")).toBeMatchedBy(
            ".js-something",
        );
        expect($("<span></span>")).toExist();
        expect($('<div id="some-id"></div>')).toHaveAttr("id", "some-id");
        expect($('<input type="checkbox" checked="checked"/>')).toHaveAttr(
            "type",
        );
        expect($('<div id="some-id"></div>')).toHaveProp("id", "some-id");
        expect($('<input type="checkbox" checked="checked"/>')).toHaveProp(
            "checked",
        );
        ...
        expect($("<div><ul></ul><h1>header</h1></div>")).toContainHtml(
            "<ul></ul>",
        );
        expect($("<div><ul></ul><h1>header</h1></div>")).toContainText(
            "header",
        );
        expect($('<div id="some-id"></div>')).toHaveId("some-id");
        expect($("<div>some text</div>")).toHaveText("some text");
        expect($('<input type="text" value="some text"/>')).toHaveValue(
            "some text",
        );
    ...
    it("Handles Style Fixtures", () => {
        ...
        jasmine
            .getStyleFixtures()
            .appendLoad("myfixture.css", "myfixture2.css");
