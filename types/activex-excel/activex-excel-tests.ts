...
const inCollection = <T = any>(
    collection: { Item(index: any): T },
    index: string | number,
): T | undefined => {
...
const newOrExistingWorksheet = () => {
    ...
    let mySheet = inCollection<Excel.Worksheet>(
        newBook.Worksheets,
        mySheetName,
    );
    if (!mySheet) {
        WScript.Echo(
            `The sheet named "${mySheetName} doesn't exist, but will be created.`,
        );
...
const moveMultipleSheets = () =>
    app.Worksheets(toSafeArray<string | number>(1, "Sheet2")).Move(4);
...
const fillCellsFromAbove = () => {
    ...
    try {
        columnA.SpecialCells(Excel.XlCellType.xlCellTypeBlanks).Formula =
            "=R[-1]C";
...
const setColumnVisibility = (visible: boolean) => {
    ...
    const sheet = inCollection<
        Excel.Worksheet | Excel.Chart | Excel.DialogSheet
    >(book.Worksheets, "Sheet1");
    if (!sheet) return;

    // search the four columns for any constants
    const checkWithin = (sheet as Excel.Worksheet)
        .Range("A1:D1")
        .SpecialCells(Excel.XlCellType.xlCellTypeConstants);
...
// highlighting the active cell, row, or column -- https://msdn.microsoft.com/en-us/vba/excel-vba/articles/highlight-the-active-cell-row-or-column
{
    ...
    ActiveXObject.on(
        ...
        "SelectionChange",
        ["Target"],
        function (this: Excel.Worksheet, prm) {
            app.ScreenUpdating = false;
...
// using selection -- https://msdn.microsoft.com/en-us/vba/excel-vba/articles/selecting-and-activating-cells
{
    ...
    (app.Selection as Excel.Range).Borders(
...
// prevent duplicate entry -- https://msdn.microsoft.com/en-us/vba/excel-vba/articles/prevent-duplicate-entries-in-a-range
{
    ...
    ActiveXObject.on(
        ...
        ["Sh", "Target"],
        function (this, prm) {
            const EvalRange = (this.ActiveSheet as Excel.Worksheet).Range(
                "A1:B20",
            );
            ...
            if (
                app.WorksheetFunction.CountIf(EvalRange, prm.Target.Value()) > 1
            ) {
...
{
    {
        ...
        const dataRange = sheet.Range(
            "A1",
            sheet.Range("A100").End(Excel.XlDirection.xlUp),
        );
        dataRange.AdvancedFilter(
        ...
        const data = sheet
            .Range("L2", sheet.Range("L100").End(Excel.XlDirection.xlUp))
            .Value() as SafeArray;
        sheet
            .Range("L1", sheet.Range("L100").End(Excel.XlDirection.xlUp))
            .ClearContents();

        const combobox = sheet.OLEObjects("ComboBox1")
            .Object as MSForms.ComboBox;
        combobox.Clear();
    ...
    // animating a sparkline -- https://msdn.microsoft.com/en-us/vba/excel-vba/articles/animate-a-sparkline
    {
        ...
        for (let i = 1; i <= 24; i++) {
            ...
            oSparkGroup.ModifySourceData(
                wks.Range(oSparkGroup.SourceData).Offset(0, 1).Address(),
            );
            WScript.Sleep(1000);
        }
    }
