function test_init() {
    ...
    const osInstanceBody: OverlayScrollbars = OverlayScrollbars(
        document.body,
        {},
    );
    ...
    if (elementById != null) {
        ...
        const osInstanceId: OverlayScrollbars = OverlayScrollbars(
            elementById,
            {},
        );
    }
    ...
    const elementsQuerySelector: NodeListOf<Element> =
        document.querySelectorAll(".os");
    if (elementsQuerySelector.length > 0) {
        // its up to the user to cast the result properly
        if (elementsQuerySelector.length === 1) {
            const osInstance: OverlayScrollbars = <OverlayScrollbars>(
                OverlayScrollbars(elementsQuerySelector, {})
            );
        } else {
            const osInstances: OverlayScrollbars[] = <OverlayScrollbars[]>(
                OverlayScrollbars(elementsQuerySelector, {})
            );
        }
    }
...
function test_getInstance() {
    ...
    const osInstanceBody: OverlayScrollbars | undefined = OverlayScrollbars(
        document.body,
    );

    // elementById is a single element or null, but the plugin might not be initialized to it if not null
    const osInstanceId: OverlayScrollbars | undefined = OverlayScrollbars(
