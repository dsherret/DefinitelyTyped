    ...
    return j(file.source)
        ...
        .replaceWith((p) => {
            ...
            const isFromProps = (name: string, resolvedScope: any) => {
                return resolvedScope.getBindings()[name].every((p: any) => {
            ...
            root.find(j.MemberExpression, {
                ...
                .filter((e) => {
                    const resolvedScope = e.scope.lookup(
                        (e.value.property as Identifier).name,
                    );
                ...
                .filter(
                    (p) => !isKeyword((p.value.property as Identifier).name),
                )
