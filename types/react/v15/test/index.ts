...
const ClassicComponent: React.ClassicComponentClass<Props> = React.createClass<
    Props,
    State
>({
...
const StatelessComponent3: React.SFC<SCProps> =
    ...
    // allows null return
    (props) =>
        props.foo ? React.DOM.div(null, props.foo, props.children) : null;
...
const statelessElement: React.SFCElement<SCProps> = React.createElement(
    StatelessComponent,
    props,
);
...
function foo3(
    child:
        ...
        | React.StatelessComponent<{ name: string }>
        | string,
) {
...
const componentElementOrNull: ModernComponent = ReactDOM.render(
    element,
    document.getElementById("anelement"),
);
...
const PropTypesSpecification: React.ComponentSpec<any, any> = {
    propTypes: {
        ...
        percentage: (
            ...
            const error = React.PropTypes.number(
                object,
                key,
                componentName,
                ...rest,
...
const mappedChildrenArray: number[] = React.Children.map<number>(
    children,
    (child) => 42,
);
...
const emptyElement2: React.ReactElement<{}> =
