...
assert(
    qrCode.options.padding === 4,
    "Default value for padding",
    "matches expected.",
    "does not match expected.",
);
assert(
    qrCode.options.width === 256,
    "Default value for width",
    "matches expected.",
    "does not match expected.",
);
assert(
    qrCode.options.height === 256,
...
assert(
    qrCode.options.color === "#000000",
    "Default value for color",
    "matches expected.",
    "does not match expected.",
);
assert(
    qrCode.options.ecl === "M",
    "Default value for ecl",
    "matches expected.",
    "does not match expected",
);

assert(
    (Array.isArray(qrCode.qrcode.modules) &&
...
assert(
    typeof qrCode.svg() === "string",
    "QRCode object",
    "generated an svg string",
