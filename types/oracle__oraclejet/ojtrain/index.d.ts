...
export interface ojTrain extends baseComponent<ojTrainSettableProperties> {
    ...
    onSelectedStepChanged:
        | ((event: JetElementCustomEvent<ojTrain["selectedStep"]>) => any)
        | null;
    onStepsChanged:
        | ((event: JetElementCustomEvent<ojTrain["steps"]>) => any)
        | null;
    ...
    getProperty<T extends keyof ojTrainSettableProperties>(
        property: T,
    ): ojTrain[T];
    getProperty(property: string): any;
    setProperty<T extends keyof ojTrainSettableProperties>(
        property: T,
        value: ojTrainSettableProperties[T],
    ): void;
    setProperty<T extends string>(
        property: T,
        value: JetSetPropertyType<T, ojTrainSettableProperties>,
    ): void;
...
export interface ojTrainEventMap
    extends baseComponentEventMap<ojTrainSettableProperties> {
...
export interface ojTrainSettableProperties
    extends baseComponentSettableProperties {
    selectedStep: string;
    steps: ojTrain.Step[];
}
export interface ojTrainSettablePropertiesLenient
    extends Partial<ojTrainSettableProperties> {
    [key: string]: any;
}
