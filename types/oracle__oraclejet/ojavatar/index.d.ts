...
export interface ojAvatar extends JetElement<ojAvatarSettableProperties> {
    ...
    src: string;
    onInitialsChanged:
        | ((event: JetElementCustomEvent<ojAvatar["initials"]>) => any)
        | null;
    onSizeChanged:
        | ((event: JetElementCustomEvent<ojAvatar["size"]>) => any)
        | null;
    onSrcChanged:
        | ((event: JetElementCustomEvent<ojAvatar["src"]>) => any)
        | null;
    ...
    getProperty<T extends keyof ojAvatarSettableProperties>(
        property: T,
    ): ojAvatar[T];
    getProperty(property: string): any;
    setProperty<T extends keyof ojAvatarSettableProperties>(
        property: T,
        value: ojAvatarSettableProperties[T],
    ): void;
    setProperty<T extends string>(
        property: T,
        value: JetSetPropertyType<T, ojAvatarSettableProperties>,
    ): void;
...
export interface ojAvatarSettablePropertiesLenient
    extends Partial<ojAvatarSettableProperties> {
    [key: string]: any;
}
