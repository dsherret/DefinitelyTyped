...
export interface ojCheckboxset<K, D>
    extends editableValue<any[], ojCheckboxsetSettableProperties<K, D>> {
    disabled: boolean;
    labelledBy: string | null;
    optionRenderer?:
        | ((param0: ojCheckboxset.OptionContext<D>) => Element)
        | null
        | undefined;
    ...
    onDisabledChanged:
        | ((
              event: JetElementCustomEvent<ojCheckboxset<K, D>["disabled"]>,
          ) => any)
        | null;
    onLabelledByChanged:
        | ((
              event: JetElementCustomEvent<ojCheckboxset<K, D>["labelledBy"]>,
          ) => any)
        | null;
    onOptionRendererChanged:
        | ((
              event: JetElementCustomEvent<
                  ojCheckboxset<K, D>["optionRenderer"]
              >,
          ) => any)
        | null;
    onOptionsChanged:
        | ((
              event: JetElementCustomEvent<ojCheckboxset<K, D>["options"]>,
          ) => any)
        | null;
    onOptionsKeysChanged:
        | ((
              event: JetElementCustomEvent<ojCheckboxset<K, D>["optionsKeys"]>,
          ) => any)
        | null;
    onRequiredChanged:
        | ((
              event: JetElementCustomEvent<ojCheckboxset<K, D>["required"]>,
          ) => any)
        | null;
    onValueChanged:
        | ((event: JetElementCustomEvent<ojCheckboxset<K, D>["value"]>) => any)
        | null;
    ...
    addEventListener<T extends keyof ojCheckboxsetEventMap<K, D>>(
        ...
        listener: (
            this: HTMLElement,
            ev: ojCheckboxsetEventMap<K, D>[T],
        ) => any,
        useCapture?: boolean,
    ): void;
    addEventListener(
    ...
    getProperty<T extends keyof ojCheckboxsetSettableProperties<K, D>>(
        property: T,
    ): ojCheckboxset<K, D>[T];
    ...
    setProperties(
        properties: ojCheckboxsetSettablePropertiesLenient<K, D>,
    ): void;
    refresh(): void;
...
export interface ojCheckboxsetSettableProperties<K, D>
...
export interface ojCheckboxsetSettablePropertiesLenient<K, D>
    extends Partial<ojCheckboxsetSettableProperties<K, D>> {
    [key: string]: any;
}
