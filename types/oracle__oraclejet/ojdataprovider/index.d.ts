...
export interface DataMapping<K, D, Kin, Din> {
    ...
    mapFilterCriterion?:
        | ((
              filterCriterion: Array<FilterOperator<D>>,
          ) => Array<FilterOperator<Din>>)
        | undefined;
    mapSortCriteria?:
        | ((sortCriteria: Array<SortCriterion<D>>) => Array<SortCriterion<Din>>)
        | undefined;
    unmapSortCriteria?:
        | ((sortCriteria: Array<SortCriterion<Din>>) => Array<SortCriterion<D>>)
        | undefined;
}
export interface DataProvider<K, D> extends EventTarget {
    containsKeys(
        parameters: FetchByKeysParameters<K>,
    ): Promise<ContainsKeysResults<K>>;
    fetchByKeys(
        parameters: FetchByKeysParameters<K>,
...
export interface DataProviderAddOperationEventDetail<K, D>
    extends DataProviderOperationEventDetail<K, D> {
...
export interface FilterOperator<D> {
    op:
        | AttributeFilterOperator.AttributeOperator
        | CompoundFilterOperator.CompoundOperator;
    filter(data: any[]): any[];
}
