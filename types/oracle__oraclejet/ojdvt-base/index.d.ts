...
export interface dvtBaseComponent<
    SP extends
        dvtBaseComponentSettableProperties = dvtBaseComponentSettableProperties,
> extends baseComponent<SP> {
    ...
    onTrackResizeChanged:
        | ((
              event: JetElementCustomEvent<dvtBaseComponent<SP>["trackResize"]>,
          ) => any)
        | null;
    addEventListener<T extends keyof dvtBaseComponentEventMap<SP>>(
        ...
        listener: (
            this: HTMLElement,
            ev: dvtBaseComponentEventMap<SP>[T],
        ) => any,
        useCapture?: boolean,
    ): void;
    addEventListener(
    ...
    getProperty<T extends keyof dvtBaseComponentSettableProperties>(
        property: T,
    ): dvtBaseComponent<SP>[T];
    ...
    setProperty<T extends string>(
        property: T,
        value: JetSetPropertyType<T, dvtBaseComponentSettableProperties>,
    ): void;
...
> extends baseComponentEventMap<SP> {
    trackResizeChanged: JetElementCustomEvent<
        dvtBaseComponent<SP>["trackResize"]
    >;
}
export interface dvtBaseComponentSettableProperties
...
export interface dvtBaseComponentSettablePropertiesLenient
    extends Partial<dvtBaseComponentSettableProperties> {
    [key: string]: any;
}
