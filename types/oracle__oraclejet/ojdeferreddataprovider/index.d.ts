...
declare class DeferredDataProvider<K, D> implements DataProvider<K, D> {
    constructor(
        dataProvider: Promise<DataProvider<K, D>>,
        capabilityFunc: (capabilityName: string) => any,
    );
    addEventListener(eventType: string, listener: EventListener): any;
    containsKeys(
        params: FetchByKeysParameters<K>,
    ...
    fetchByOffset(
        params: FetchByOffsetParameters<D>,
    ): Promise<FetchByOffsetResults<K, D>>;
    fetchFirst(
        params?: FetchListParameters<D>,
