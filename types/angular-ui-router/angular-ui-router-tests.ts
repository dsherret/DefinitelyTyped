...
myApp.config(
    (
        ...
        var matcher: ng.ui.IUrlMatcher =
            $urlMatcherFactory.compile("/foo/:bar?param1");
        ...
        $urlMatcherFactory.type("fullType", {
            ...
            is: (val) =>
                angular.isNumber(val) && isFinite(val) && val % 1 === 0,
            pattern: /\d+/,
        });
...
class UrlLocatorTestService implements IUrlLocatorTestService {
    ...
    constructor(
        ...
        $rootScope.$on("$locationChangeSuccess", (event: ng.IAngularEvent) =>
            this.onLocationChangeSuccess(event),
        );
        $rootScope.$on(
            "$stateNotFound",
            (
    ...
    private onLocationChangeSuccess(event: ng.IAngularEvent) {
        if (!this.currentUser) {
            ...
            this.$http({ url: "/api/me", method: "GET" }).then(
