...
function test_dirtyFlag() {
    ...
    var self: any;
    this.dirtyFlag = new ko.DirtyFlag(self.firstName, self.lastName);
...
function test_full() {
        ...
        ko.asyncCommand = function (options) {
            ...
            self.canExecute = ko.computed(function () {
                return canExecuteDelegate
                    ? canExecuteDelegate(self.isExecuting())
                    : !self.isExecuting();
            });
            self.execute = function (arg1, arg2) {
                if (!self.canExecute()) return;
    ...
    my.TwitterService = function () {
        var me = this,
            twitterUrl =
                "https://api.twitter.com/1/statuses/user_timeline/{name}.json?callback=?&count={count}";
