...
namespace application {
    ...
    myApp.controller("mainController", [
        ...
        function ($scope: TestScope, datasource: DatasourceTest) {
            var firstListAdapter: ng.ui.IScrollAdapter,
                secondListAdapter: ng.ui.IScrollAdapter;
            $scope["datasource"] = datasource;

            $scope["updateList1"] = (): void => {
                firstListAdapter.applyUpdates(
                    (item: IItem, scope: ng.IRepeatScope) => {
                        return (item.content += " *");
                    },
                );
            };
            ...
            $scope["updateList2"] = (): void => {
                secondListAdapter.applyUpdates(
                    (item: IItem, scope: ng.IRepeatScope) => {
                        return (item.content += " *");
                    },
                );
