import { Client, CustomTypesConfig, types } from "pg";
...
const handleFn: ResultCallback<string> = (
    err: Error | undefined,
    rows: string[],
) => {
    if (err) throw err;
...
const promiseHandle = client.query(
    new Cursor<{ name: string }>("SELECT $1::text as name", ["brianc"]),
);
promiseHandle
    .read(100)
    .then((rows) => {
        ...
        if (typeof row.name !== "string")
            throw new Error("Generic type does not work");
        return rows;
    })
