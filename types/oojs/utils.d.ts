declare namespace OO {
    ...
    type GetProp<T, K extends ValidKey> = T extends Partial<Record<K, any>>
        ? T[K]
        : undefined;
    ...
    type LeavesOf<T, D extends number> = T extends NonPlainObject
        ? T
        : Extract<NodesOf<T, D>, NonPlainObject>;

    type FunctionFieldsOf<
        T,
    ...
    type EventHandler<
        C,
        F extends (...args: any[]) => any = (this: C, ...args: any[]) => void,
    > = FunctionFieldsOf<C, F> | F;
    ...
    type EventConnectionMap<T extends object, C, M extends object> = {
        [K in keyof T]: EventConnectionMapEntry<
            C,
            (M & Record<string | number | symbol, any[]>)[K],
            T[K]
        >;
    };
