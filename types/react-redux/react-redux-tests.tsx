...
function MapNothingAndMerge() {
    ...
    const Test = connect(null, null, mergeProps)(TestComponent);
...
// only first argument
connect<CounterStateProps, {}, {}, CounterState>(() => mapStateToProps)(
    Counter,
);
...
const AppWrap = (
    props: DispatchProp & { children?: React.ReactNode | undefined },
) => <div />;
...
const ADecoratedTestComponent: React.NamedExoticComponent<TestProp> =
...
function TestTOwnPropsInference() {
    ...
    class OwnPropsComponent extends React.Component<
        StateProps & OwnProps & DispatchProp
    > {
        render() {
            return <div />;
        }
...
function TestMergedPropsInference() {
    ...
    const ConnectedWithOwnAndState: React.NamedExoticComponent<OwnProps> =
        connect<
            StateProps,
            // eslint-disable-next-line @typescript-eslint/no-invalid-void-type
            void,
    ...
    const ConnectedWithOwnAndDispatch: React.NamedExoticComponent<OwnProps> =
        connect<
            // eslint-disable-next-line @typescript-eslint/no-invalid-void-type
            void,
            DispatchProps,
    ...
    // eslint-disable-next-line @typescript-eslint/no-invalid-void-type
    // eslint-disable-next-line @typescript-eslint/no-invalid-void-type
    const ConnectedWithOwn: React.NamedExoticComponent<OwnProps> = connect<
        void,
        void,
        OwnProps,
        MergedProps
...
function TestProviderContext() {
    ...
    const initialContextValue: ReactReduxContextValue = {
        storeState: null,
        store,
    };
...
function testConnectReturnType() {
    ...
    const myHoc1 = <P,>(C: React.ComponentClass<P>): React.ComponentType<P> =>
        C;
    // @ts-expect-error
    myHoc1(Test);
