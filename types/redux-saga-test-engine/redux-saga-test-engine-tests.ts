...
const putEffects = testEngine(
    testGenerator,
    [[put({ type: "TEST", payload: "arg" }), null]],
    "arg",
);
// $ExpectType (Effect<"PUT", any> | Effect<"PUT", any>[])[]
putEffects;

const puts = collectPuts(
    testGenerator,
    [[put({ type: "TEST", payload: "arg" }), null]],
    "arg",
);
...
const callsAndPuts = collectCallsAndPuts(
    testGenerator,
    [[put({ type: "TEST", payload: "arg" }), null]],
    "arg",
);
...
const incorrectEffects = incorrectEngine(
    incorrectGenerator,
    [[call(identity, 123), null]],
    123,
);
