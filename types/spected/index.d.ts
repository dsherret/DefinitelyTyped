declare function spected<
    ROOTINPUT,
    SPEC extends SpecValue<ROOTINPUT, ROOTINPUT> = SpecValue<
        ROOTINPUT,
        ROOTINPUT
    >,
>(spec: SPEC, input: ROOTINPUT): Result<ROOTINPUT, SPEC>;
...
export type Spec<INPUT, ROOTINPUT = any> = [
    Predicate<INPUT, ROOTINPUT>,
    ErrorMsg<INPUT>,
];

export type SpecArray<INPUT, ROOTINPUT = any> = Array<Spec<INPUT, ROOTINPUT>>;

export type SpecFunction<INPUT, ROOTINPUT = any> = [INPUT] extends [
...
export type SpecObject<INPUT, ROOTINPUT = any> = Partial<{
    [key in keyof INPUT]: SpecValue<INPUT[key], ROOTINPUT>;
}>;
...
export type Result<INPUT, SPEC> = {
    [key in keyof INPUT]: true | any[] | Result<INPUT[key], any>;
};
