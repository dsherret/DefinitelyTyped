...
function nodeJS(session: autobahn.Session) {
    var sessionObservable = Rx.Observable.fromConnection({
...
function connection() {
    var connectionSubscription = Rx.Observable.fromConnection({
        url: "ws://localhost:9000",
        realm: "realm1",
...
function subscribe_to_topics(
    ...
    var topic = Rx.Observable.subscribeAsObservable(
        session,
        "wamp.my.foo",
        options,
        openObserver,
...
function new_in_version3(onResult: Rx.Observer<autobahn.ISubscription>) {
    Rx.Observable.subscribeAsObservable(
        Rx.Observable.fromConnection({
            url: "ws://myconnectionurl:9090",
            realm: "realm1",
        }),
        "wamp.my.foo",
        undefined,
...
function new_in_version5(fooObserver: Rx.Observer<IWampEvent>) {
    var connection = Rx.Observable.fromConnection({
        url: "ws://myconnectionurl:9090",
        realm: "realm1",
    });

    // You can subscribe to as many items as you want
    var subscriber = Rx.Observable.subscriber(connection)
...
function publishing_to_topic(session: autobahn.Session) {
    var published = Rx.Observable.publishAsObservable(
        session,
        "wamp.my.foo",
        [42],
        { key: "value" },
...
function registering_methods(
    session: autobahn.Session,
    endpoint: autobahn.RegisterEndpoint,
) {
    var registration = Rx.Observable.registerAsObservable(
        session,
        "wamp.my.add",
        endpoint,
        {},
...
function new_in_version3_registration(
    myUrl: string,
    endpoint: autobahn.RegisterEndpoint,
) {
    var connection = Rx.Observable.fromConnection({
...
function calling_methods(session: autobahn.Session) {
    var caller = Rx.Observable.callAsObservable<number>(
        session,
        "wamp.my.add",
        {},
    );

    var addResult = caller([2, 3], {}).subscribe(function (value) {
        // => 5
...
function weather_station_monitor(session: autobahn.Session) {
    ...
    var sensorReadings = Rx.Observable.subscribeAsObservable(
        session,
        "weather.sensor",
    );

    // A remote service for analyzing our readings, it might be aggregating across several different sources
    var analyzer = Rx.Observable.callAsObservable<IWeatherReadings>(
        session,
        "weather.forecast.compute",
    );

    // Home control settings
    var desiredTemperature = Rx.Observable.subscribeAsObservable(
        session,
        "temperature.indoors.desired",
    ).map((x) => x.args[0] as number);

    var dailyForecast = sensorReadings
        ...
        .tap((readings) => {
            ...
            Rx.Observable.publishAsObservable(
                session,
                "weather.visualizer.daily",
                readings,
            );
        })
    ...
    dailyForecast
        ...
        .subscribe(
            Rx.Observable.publishAsObservable.bind(
                null,
                session,
                "weather.warnings.klaxon",
            ),
        );
    ...
    dailyForecast
        ...
        .combineLatest(desiredTemperature, (actual, desired) =>
