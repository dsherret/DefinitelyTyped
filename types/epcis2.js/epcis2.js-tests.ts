...
const buildSensorReportElementExample = (): SensorReportElement => {
    ...
    return new SensorReportElement()
        .setType(cbv.sensorMeasurementTypes.temperature)
        .setDeviceID(
            `urn:epc:id:giai:4000001.${Math.floor(Math.random() * 999)}`,
        )
        ...
        .addExtension(
            "example:someFurtherMetadata",
            "someText",
        ) as SensorReportElement;
};
...
const buildBizLocationExample = (): BizLocation => {
    return new BizLocation().setId(
        "urn:epc:id:sgln:0012345.11111." + Math.floor(Math.random() * 9),
    );
};

const buildBizTransactionExample = (): BizTransactionElement => {
    return new BizTransactionElement()
        .setType(cbv.businessTransactionTypes.po)
        .setBizTransaction(
            "urn:epc:id:gdti:0614141.00001.1618" +
...
const buildQuantityElementExample = () => {
    return new QuantityElement()
        .setEpcClass("urn:epc:class:lgtin:4012345.012345.9911111")
...
const buildSourceElementExample = () => {
    return new SourceElement()
        .setType(cbv.sourceDestinationTypes.location)
        .setSource(
            "urn:epc:id:sgln:4012345.00225." + Math.floor(Math.random() * 9),
        );
};
...
const buildSensorElementExample = (): SensorElement => {
    const sensorReportList = [
        buildSensorReportElementExample(),
        buildSensorReportElementExample(),
    ];

    return new SensorElement()
        .setSensorMetadata(buildSensorMetadataExample())
...
const buildPersistentDispositionExample = (): PersistentDisposition => {
    return new PersistentDisposition()
        .addSet(cbv.dispositions.completeness_verified)
...
const buildObjectEvent = (): ObjectEvent => {
    const bizTransactionList = [
        buildBizTransactionExample(),
        buildBizTransactionExample(),
    ];
    const quantityList = [
        buildQuantityElementExample(),
        buildQuantityElementExample(),
    ];
    const sourceList = [
        buildSourceElementExample(),
        buildSourceElementExample(),
    ];
    const destinationList = [
        buildDestinationElementExample(),
        buildDestinationElementExample(),
    ];
    const sensorElementList = [
        buildSensorElementExample(),
        buildSensorElementExample(),
    ];
        ...
        sensorElementList: [
            {
                sensorMetadata: {
                    ...
                    dataProcessingMethod:
                        "https://example.com/gdti/4012345000054987",
