...
interface TodoAngularMeteorObject
    extends ITodo,
        angular.meteor.AngularMeteorObject<ITodo> {}
...
app.controller("mainCtrl", [
    "$scope",
    "$meteor",
    ($scope: CustomScope, $meteor: angular.meteor.IMeteorService) => {
        ...
        $scope.notAutoTodos = $meteor
            .collection(Todos, false)
            .subscribe("publicTodos");

        $scope.todoNotAuto = <TodoAngularMeteorObject>(
            $meteor.object(Todos, "TodoID", false)
        );
        $scope.todoSubscribed = <TodoAngularMeteorObject>(
            $meteor.object(Todos, "TodoID").subscribe("todos")
        );
        ...
        $meteor
            .call<ITodo>("subscribe", $scope.todo._id, $scope.currentUser._id)
            .then(
                (data) => {
                    // Handle success
