...
declare class SocketServer extends EventEmitter {
    ...
    listeners(
        event: "connection",
    ): Array<(socket: Socket, request: http.IncomingMessage) => void>;
    listeners(event: "error"): Array<(error: Error) => void>;
    listeners(
        event: "headers",
    ): Array<(headers: string[], request: http.IncomingMessage) => void>;
    listeners(event: "listening"): Array<() => void>;

    off(
        event: "connection",
        cb: (socket: Socket, request: http.IncomingMessage) => void,
    ): this;
