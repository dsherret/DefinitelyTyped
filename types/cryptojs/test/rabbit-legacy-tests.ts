        ...
        Y.Test.Runner.add(
            new Y.Test.Case({
                ...
                testMultiPart: function () {
                    var rabbit = C.algo.RabbitLegacy.createEncryptor(
                        C.enc.Hex.parse("00000000000000000000000000000000"),
                    );
                    var ciphertext1 = rabbit.process(
                        C.enc.Hex.parse("000000000000"),
                    );
                    var ciphertext2 = rabbit.process(
                        C.enc.Hex.parse("0000000000"),
                    );
                    var ciphertext3 = rabbit.process(
                        C.enc.Hex.parse("0000000000"),
                    );
                ...
                testHelper: function () {
                    ...
                    // Test
                    Y.Assert.areEqual(
                        C.algo.RabbitLegacy.createEncryptor(C.MD5("Jefe"))
                            .finalize("Hi There")
                            .toString(),
                        C.RabbitLegacy.encrypt(
                            "Hi There",
