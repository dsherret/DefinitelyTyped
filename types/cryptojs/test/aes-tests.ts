        ...
        Y.Test.Runner.add(
            new Y.Test.Case({
                ...
                testMultiPart: function () {
                    var aes = C.algo.AES.createEncryptor(
                        C.enc.Hex.parse("000102030405060708090a0b0c0d0e0f"),
                        {
                            mode: C.mode.ECB,
                    ...
                    var ciphertext1 = aes.process(
                        C.enc.Hex.parse("001122334455"),
                    );
                    var ciphertext2 = aes.process(
                        C.enc.Hex.parse("66778899aa"),
                    );
                    var ciphertext3 = aes.process(
                ...
                testHelper: function () {
                    ...
                    // Test
                    Y.Assert.areEqual(
                        C.algo.AES.createEncryptor(C.SHA256("Jefe"), {
                            mode: C.mode.ECB,
                            padding: C.pad.NoPadding,
                        })
                            .finalize("Hi There")
