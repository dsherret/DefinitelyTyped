...
factory.define<User>(
    ...
    {},
    {
        ...
        score: scoreSequence,
        email: factory.seq<string>(
            "User.email",
            (num) => `email-${1}@users.com`,
        ),
        roles: factory.assocMany("Role", 3, "id"),
        creditCard: factory.assocAttrs(
            "CreditCard",
            "creditCard",
            { number: "1234" },
            { option: true },
...
factory
    .build<User>("user", {}, { isAdmin: true })
    .then((user) => user.username);

// Testing buildMany, with and without attributes and options
factory
    .buildMany<User>("user", 3)
    .then((users) => users.map((user) => user.username));
factory
    .buildMany<User>("user", 3, { username: "John McClane" })
    .then((users) => users.map((user) => user.username));
factory
    .buildMany<User>("user", 3, {}, { isAdmin: true })
    .then((users) => users.map((user) => user.username));
factory
    .buildMany<User>("user", 1, [{}], [{ isAdmin: true }])
    .then((users) => users.map((user) => user.username));
