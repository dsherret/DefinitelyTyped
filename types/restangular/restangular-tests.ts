...
myApp.config((RestangularProvider: restangular.IProvider) => {
    ...
    RestangularProvider.extendCollection(
        "accounts",
...
myApp.controller(
    "TestCtrl",
    ($scope: MyAppScope, Restangular: restangular.IService) => {
        ...
        Restangular.setErrorInterceptor(function (response) {
            ...
            console.error(
                "" + response.status + " " + response.data + " " + location,
            );
        });
        ...
        Restangular.addElementTransformer(
            "accounts",
            false,
            function (elem: any) {
                elem.accountName = "Changed";
        ...
        var accountClone: restangular.IElement = Restangular.one(
            "accounts",
            123,
        ).clone();
        ...
        baseAccounts.getList().then(
            function (accounts) {
                ...
                $scope.loggedInPlaces = firstAccount.getList(
                    "places",
                    { query: "param" },
                    { "x-user": "mgonto" },
                );
        ...
        var myRestangular = Restangular.withConfig(
            (configurer: restangular.IProvider) => {
                ...
                configurer.setRequestInterceptor(
                    function (element, operation, route, url) {},
                );

                configurer.addElementTransformer(
                    "accounts",
                    false,
                    function (elem: any) {
                        elem.accountName = "Changed";
