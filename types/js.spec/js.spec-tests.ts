...
const result = S.conform(
    S.spec.map("dancing", { field: S.spec.string }),
    "not a map",
);
...
const {
    ...
    value,
    predicate,
}: { path: string[]; via: string[]; value: any; predicate: S.PredFn } =
    problems[0];
...
const orSpec: S.Spec = S.spec.or("or test", {
    ball: (value: any) => value === "whale",
    fish: S.spec.number,
});

const nilableSpec: S.Spec = S.spec.nilable(
    "nilable test",
    (value: any) => false,
);

const collectionSpec: S.Spec = S.spec.collection(
    "collection test",
    S.spec.positive,
);

const collection2Spec: S.Spec = S.spec.collection(
    "collection test",
    S.spec.string,
    { [S.symbol.count]: 3 },
);

const tupleSpec: S.Spec = S.spec.tuple(
    "tuple test",
    S.spec.bool,
    S.spec.date,
    S.spec.array,
...
const mapSpec: S.Spec = S.spec.map("map test", {
    email: S.spec.string,
    [S.symbol.optional]: { name: S.spec.string },
});

const oneOfSpec: S.Spec = S.spec.oneOf("oneOf test", "a", "b", "c");

const predicateSpec: S.Spec = S.spec.predicate(
    "predicate test",
    (value) => true,
);
