...
export interface AbortSignalLike {
    addEventListener: (
        type: "abort",
        listener: (this: AbortSignalLike, event: any) => any,
    ) => void;
}
...
export type AnyReadable<TType = any, TMap = TType, TByte = TMap> =
    | NodeJSReadable
    | Readable<TType, TMap, TByte>;
...
export interface StreamOptions<
    TStream extends Stream<TByteType>,
    TByteType = any,
> {
...
export type EventName<TEvents extends Events> = TEvents extends undefined
    ? string | symbol
    : keyof TEvents;
export type EventListener<
    TEvents extends Events,
    TEvent extends string | symbol | number,
> = TEvents extends undefined
    ? (...args: any[]) => void
...
> extends EventEmitter {
    ...
    addListener<TEvent extends EventName<TEvents>>(
        event: TEvent,
    ...
    removeListener<TEvent extends EventName<TEvents>>(
        event: TEvent,
        listener: EventListener<TEvents, TEvent>,
    ): this;
    ...
    listeners<TEvent extends EventName<TEvents>>(
        event: TEvent,
    ): Array<EventListener<TEvents, TEvent>>;
    rawListeners<TEvent extends EventName<TEvents>>(
        event: TEvent,
    ): Array<EventListener<TEvents, TEvent>>;
    ...
    // Added in Node 6...
    prependListener<TEvent extends EventName<TEvents>>(
        event: TEvent,
        listener: EventListener<TEvents, TEvent>,
    ): this;
...
export type MapFunction<TThis, TIn, TOut> =
    | ((this: TThis, input: TIn) => TOut)
    | null
    | undefined;
export type ByteLengthFunction<TThis, TData> =
    | ((this: TThis, data: TData) => number)
    | null
    | undefined;
...
> extends Stream<TByteType, TReadable, TWritable, TEvents> {
    ...
    pipe<
        TTarget extends AnyWritable<TMapType, any, any> = AnyWritable<
            TMapType,
            any,
            any
        >,
    >(dest: TTarget, cb?: Callback): TTarget;
    read(): TMapType;
...
> extends Stream<TByteType, TReadable, TWritable, TEvents> {
    constructor(
        opts?: WritableOptions<
            Writable<TType, TMapType, TByteType>,
            TType,
            TMapType,
            TByteType
        >,
...
export type DuplexEvents<TWriteType, TReadType> = ReadableEvents<TReadType> &
...
export class PassThrough<TRead = any, TWrite = any> extends Transform<
    TRead,
    TWrite
> {
    constructor(
        opts?: TransformOptions<Transform<TRead, TWrite>, TRead, TWrite>,
    );
}
