...
var dummyTemplateEngine = function (templates?) {
    ...
    this.renderTemplateSource = function (
        ...
        templateText = options.showParams
            ? templateText + ", data=" + data + ", options=" + options
            : templateText;
        ...
        // with (bindingContext)
        {
            // with (data || {})
            {
                // with (options.templateRenderingVariablesInScope || {})
                {
                    ...
                    result = templateText.replace(
                        /\[renderTemplate\:(.*?)\]/g,
                        function (match, templateName) {
                            return ko.renderTemplate(
                                templateName,
                                data,
                                options,
                            );
    ...
    this.rewriteTemplate = function (template, rewriterCallback) {
        ...
        if (typeof templateSource.text() != "function") {
            return ko.templateEngine.prototype.rewriteTemplate.call(
                this,
                template,
                rewriterCallback,
            );
...
describe("Templating", function () {
    ...
    it("Should not rerender DOM element if observable accessed in 'afterRender' callaback is changed", function () {
        ...
        ko.setTemplateEngine(
            new dummyTemplateEngine({ someTemplate: myTemplate }),
        );
        ko.renderTemplate(
            "someTemplate",
            {},
    ...
    it("Should be able to render a template using data-bind syntax", function () {
        ko.setTemplateEngine(
            new dummyTemplateEngine({ someTemplate: "template output" }),
        );
        testNode.innerHTML =
    ...
    it("Should be able to tell data-bind syntax which object to pass as data for the template (otherwise, uses viewModel)", function () {
        ...
        testNode.innerHTML =
            "<div data-bind='template: { name: \"someTemplate\", data: someProp }'></div>";
    ...
    it("Should stop tracking inner observables immediately when the container node is removed from the document", function () {
        ...
        testNode.innerHTML =
            "<div data-bind='template: { name: \"someTemplate\", data: someProp }'></div>";
        ko.applyBindings(
            { someProp: { childProp: innerObservable } },
            testNode,
        );
    ...
    it("Should be able to pick template via an observable model property", function () {
        ...
        expect(testNode.childNodes[0].innerHTML).toEqual(
            "First template output",
        );

        chosenTemplate("secondTemplate");
        expect(testNode.childNodes[0].innerHTML).toEqual(
            "Second template output",
        );
    });
    ...
    it("Should be able to pick template as a function of the data item using data-bind syntax, with the binding context available as a second parameter", function () {
        ...
        testNode.innerHTML =
            "<div data-bind='template: { name: templateSelectorFunction, data: someProp }'></div>";
        ko.applyBindings(
            {
    ...
    it("Should be able to chain templates, rendering one from inside another", function () {
        ko.setTemplateEngine(
            new dummyTemplateEngine({
                outerTemplate:
                    "outer template output, [renderTemplate:innerTemplate]", // [renderTemplate:...] is special syntax supported by dummy template engine
                innerTemplate:
                    "inner template output <span data-bind='text: 123'></span>",
            }),
        );
        testNode.innerHTML =
            "<div data-bind='template:\"outerTemplate\"'></div>";
        ko.applyBindings(null, testNode);
        expect(testNode.childNodes[0]).toContainHtml(
            "outer template output, inner template output <span>123</span>",
        );
    });
    ...
    it("Should stop tracking inner observables referenced by a chained template as soon as the chained template output node is removed from the document", function () {
        ...
        testNode.innerHTML =
            "<div data-bind='template: { name: \"outerTemplate\", data: someProp }'></div>";
        ko.applyBindings(
            { someProp: { childProp: innerObservable } },
            testNode,
        );
        ...
        expect(innerObservable.getSubscriptionsCount()).toEqual(1);
        let innerTemplateOutputEl = document.getElementById(
            "innerTemplateOutput",
        );
    ...
    it("Data binding syntax should be able to reference variables put into scope by the template engine", function () {
        ...
        ko.renderTemplate(
            "someTemplate",
            null,
            { templateRenderingVariablesInScope: { message: "hello" } },
            testNode,
        );
    ...
    it("Data binding syntax should be able to use $context in binding value to refer to the context object", function () {
        ko.setTemplateEngine(
            new dummyTemplateEngine({
                someTemplate:
                    "<div data-bind='text: $context.$data === $data'></div>",
            }),
        );
    ...
    it("Data binding syntax should defer evaluation of variables until the end of template rendering (so bindings can take independent subscriptions to them)", function () {
        ko.setTemplateEngine(
            new dummyTemplateEngine({
                someTemplate:
                    "<input data-bind='value:message' />[js: message = 'goodbye'; undefined; ]",
            }),
        );
        ko.renderTemplate(
            "someTemplate",
    ...
    it("Data binding syntax should use the template's 'data' object as the viewModel value (so 'this' is set correctly when calling click handlers etc.)", function () {
        ko.setTemplateEngine(
            new dummyTemplateEngine({
                someTemplate:
                    "<button data-bind='click: someFunctionOnModel'>click me</button>",
            }),
        );
        var viewModel = {
    ...
    it("Data binding syntax should permit nested templates, and only bind inner templates once", function () {
        ...
        ko.bindingProvider.instance = {
            nodeHasBindings: function (node) {
                return (
                    (<Element>node).tagName == "EM" ||
                    originalBindingProvider.nodeHasBindings(node)
                );
            },
        ...
        testNode.innerHTML =
            "<div data-bind='template: { name: \"outerTemplate\", bypassDomNodeWrap: true }'></div>";
    ...
    it("Data binding syntax should support 'foreach' option, whereby it renders for each item in an array but doesn't rerender everything if you push or splice", function () {
        ...
        testNode.innerHTML =
            "<div data-bind='template: { name: \"itemTemplate\", foreach: myCollection }'></div>";

        ko.applyBindings({ myCollection: myArray }, testNode);
        expect(testNode.childNodes[0]).toContainHtml(
            "<div>the item is bob</div><div>the item is frank</div>",
        );
    ...
    it("Data binding 'foreach' option should apply bindings within the context of each item in the array", function () {
        ...
        ko.setTemplateEngine(
            new dummyTemplateEngine({
                itemTemplate:
                    "The item is <span data-bind='text: personName'></span>",
            }),
        );
        testNode.innerHTML =
            "<div data-bind='template: { name: \"itemTemplate\", foreach: myCollection }'></div>";
        ...
        ko.applyBindings({ myCollection: myArray }, testNode);
        expect(testNode.childNodes[0]).toContainHtml(
            "the item is <span>bob</span>the item is <span>frank</span>",
        );
    });
    ...
    it("Data binding 'foreach' should handle templates in which the very first node has a binding", function () {
        ...
        testNode.innerHTML =
            "<div data-bind=\"template: { name: 'mytemplate', foreach: items }\"></div>";
    ...
    it("Data binding 'foreach' should handle chained templates in which the very first node has a binding", function () {
        ...
        ko.setTemplateEngine(
            new dummyTemplateEngine({
                outerTemplate:
                    "<div data-bind='text: $data'></div>[renderTemplate:innerTemplate]x", // [renderTemplate:...] is special syntax supported by dummy template engine
                innerTemplate: "inner <span data-bind='text: 123'></span>",
            }),
        );
        testNode.innerHTML =
            "<div data-bind=\"template: { name: 'outerTemplate', foreach: items }\"></div>";

        // Bind against initial array containing one entry.
        var myArray = ko.observableArray(["original"]);
        ko.applyBindings({ items: myArray }, testNode);
        expect(testNode.childNodes[0]).toContainHtml(
            "<div>original</div>inner <span>123</span>x",
        );

        // Now replace the entire array contents with one different entry.
        myArray(["new"]);
        expect(testNode.childNodes[0]).toContainHtml(
            "<div>new</div>inner <span>123</span>x",
        );
    });
    ...
    it("Data binding 'foreach' option should apply bindings with an $index in the context", function () {
        ...
        ko.setTemplateEngine(
            new dummyTemplateEngine({
                itemTemplate:
                    "The item # is <span data-bind='text: $index'></span>",
            }),
        );
        testNode.innerHTML =
            "<div data-bind='template: { name: \"itemTemplate\", foreach: myCollection }'></div>";
        ...
        ko.applyBindings({ myCollection: myArray }, testNode);
        expect(testNode.childNodes[0]).toContainHtml(
            "the item # is <span>0</span>the item # is <span>1</span>",
        );
    });
    ...
    it("Data binding 'foreach' option should update bindings that reference an $index if the list changes", function () {
        ...
        expect(testNode.childNodes[0]).toContainHtml(
            "the item <span>bob</span>is <span>0</span>",
        );
    ...
    it('Data binding \'foreach\' option should accept array with "undefined" and "null" items', function () {
        ...
        ko.setTemplateEngine(
            new dummyTemplateEngine({
                itemTemplate:
                    "The item is <span data-bind='text: String($data)'></span>",
            }),
        );
        testNode.innerHTML =
            "<div data-bind='template: { name: \"itemTemplate\", foreach: myCollection }'></div>";
        ...
        ko.applyBindings({ myCollection: myArray }, testNode);
        expect(testNode.childNodes[0]).toContainHtml(
            "the item is <span>undefined</span>the item is <span>null</span>",
        );
    });
    ...
    it("Data binding 'foreach' option should update DOM nodes when a dependency of their mapping function changes", function () {
        ...
        ko.setTemplateEngine(
            new dummyTemplateEngine({
                itemTemplate:
                    "<div>The item is [js: ko.utils.unwrapObservable(personName)]</div>",
            }),
        );
        testNode.innerHTML =
            "<div data-bind='template: { name: \"itemTemplate\", foreach: myCollection }'></div>";
        ...
        myArray.splice(1, 1);
        expect(testNode.childNodes[0]).toContainHtml(
            "<div>the item is bob</div><div>the item is another</div>",
        );
    ...
    it("Data binding 'foreach' option should treat a null parameter as meaning 'no items'", function () {
        ...
        ko.setTemplateEngine(
            new dummyTemplateEngine({ itemTemplate: "hello" }),
        );
        testNode.innerHTML =
            "<div data-bind='template: { name: \"itemTemplate\", foreach: myCollection }'></div>";
    ...
    it("Data binding 'foreach' option should accept an \"as\" option to define an alias for the iteration variable", function () {
        ...
        var myArray = ko.observableArray(["A", "B"]);
        ko.setTemplateEngine(
            new dummyTemplateEngine({ itemTemplate: "[js:myAliasedItem]" }),
        );
        testNode.innerHTML =
    ...
    it("Data binding 'foreach' option should stop tracking inner observables when the container node is removed", function () {
        ...
        ko.setTemplateEngine(
            new dummyTemplateEngine({
                itemTemplate:
                    "The item is [js: ko.utils.unwrapObservable(obsVal)]",
            }),
        );
        testNode.innerHTML =
            "<div data-bind='template: { name: \"itemTemplate\", foreach: myCollection }'></div>";
    ...
    it("Data binding syntax should omit any items whose '_destroy' flag is set (unwrapping the flag if it is observable)", function () {
        ...
        ko.applyBindings({ myCollection: myArray }, testNode);
        expect(testNode.childNodes[0]).toContainHtml(
            "<div>someprop=1</div><div>someprop=3</div><div>someprop=4</div>",
        );
    });
    ...
    it("Data binding syntax should include any items whose '_destroy' flag is set if you use includeDestroyed", function () {
        ...
        ko.applyBindings({ myCollection: myArray }, testNode);
        expect(testNode.childNodes[0]).toContainHtml(
            "<div>someprop=1</div><div>someprop=2</div><div>someprop=3</div>",
        );
    });
    ...
    it('Data binding syntax should support "if" condition', function () {
        ...
        testNode.innerHTML =
            '<div data-bind=\'template: { name: "myTemplate", "if": myProp }\'></div>';

        var viewModel = {
            myProp: ko.observable<{ childProp: string } | null>({
                childProp: "abc",
            }),
        };
    ...
    it('Data binding syntax should support "ifnot" condition', function () {
        ...
        testNode.innerHTML =
            "<div data-bind='template: { name: \"myTemplate\", ifnot: shouldHide }'></div>";
    ...
    it('Data binding syntax should support "if" condition in conjunction with foreach', function () {
        ...
        ko.applyBindings(viewModel, testNode);
        expect(testNode.childNodes[0].childNodes[0].nodeValue).toEqual(
            "Value: abc",
        );
        expect(testNode.childNodes[0].childNodes[1].nodeValue).toEqual(
            "Value: abc",
        );
        expect(testNode.childNodes[0].childNodes[2].nodeValue).toEqual(
            "Value: abc",
        );
    ...
    it("Should be able to populate checkboxes from inside templates, despite IE6 limitations", function () {
        ...
        ko.renderTemplate(
            "someTemplate",
            null,
            { templateRenderingVariablesInScope: { isChecked: true } },
            testNode,
        );
    ...
    it("Should be able to populate radio buttons from inside templates, despite IE6 limitations", function () {
        ko.setTemplateEngine(
            new dummyTemplateEngine({
                someTemplate:
                    "<input type='radio' name='somename' value='abc' data-bind='checked:someValue' />",
            }),
        );
        ko.renderTemplate(
            "someTemplate",
            null,
            { templateRenderingVariablesInScope: { someValue: "abc" } },
            testNode,
        );
        expect(testNode.childNodes[0].checked).toEqual(true);
    });
    ...
    it("Should be able to render a different template for each array entry by passing a function as template name, with the array entry's binding context available as a second parameter", function () {
        ...
        var getTemplate = function (dataItem, bindingContext) {
            ...
            return dataItem.preferredTemplate == 1
                ? "firstTemplate"
                : "secondTemplate";
        };
        ko.applyBindings(
    ...
    it("Data binding 'templateOptions' should be passed to template", function () {
        ...
        ko.setTemplateEngine(
            new dummyTemplateEngine({
                myTemplate:
                    "<div>Person [js:name] has additional property [js:templateOptions.myAdditionalProp]</div>",
            }),
        );
        testNode.innerHTML =
    ...
    it("If the template binding is updated, should dispose any template subscriptions previously associated with the element", function () {
        ...
        testNode.innerHTML =
            "<div data-bind='template: {name: \"myTemplate\", data: subModel}'></div>";
        ko.applyBindings(myModel, testNode);
    ...
    it("Should be able to specify a template engine instance using data-bind syntax", function () {
        ko.setTemplateEngine(
            new dummyTemplateEngine({ theTemplate: "Default output" }),
    ...
    it("Should be able to bind $data to an alias using 'as'", function () {
        ko.setTemplateEngine(
            new dummyTemplateEngine({
                myTemplate:
                    "ValueLiteral: [js:item.prop], ValueBound: <span data-bind='text: item.prop'></span>",
            }),
        );
        testNode.innerHTML =
            '<div data-bind=\'template: { name: "myTemplate", data: someItem, as: "item" }\'></div>';
        ko.applyBindings({ someItem: { prop: "Hello" } }, testNode);
        expect(testNode.childNodes[0]).toContainText(
            "ValueLiteral: Hello, ValueBound: Hello",
        );
    });
    ...
    it('Data-bind syntax should expose parent binding context as $parent if binding with an explicit "data" value', function () {
        ...
        testNode.innerHTML =
            "<div data-bind='template: { name: \"myTemplate\", data: someItem }'></div>";
    ...
    it("Data-bind syntax should expose all ancestor binding contexts as $parents", function () {
        ko.setTemplateEngine(
            new dummyTemplateEngine({
                outerTemplate:
                    "<div data-bind='template: { name:\"middleTemplate\", data: middleItem }'></div>",
                middleTemplate:
                    "<div data-bind='template: { name: \"innerTemplate\", data: innerItem }'></div>",
                innerTemplate:
                    "(Data:[js:$data.val], Parent:[[js:$parents[0].val]], Grandparent:[[js:$parents[1].val]], Root:[js:$root.val], Depth:[js:$parents.length])",
            }),
        );
        testNode.innerHTML =
            "<div data-bind='template: { name: \"outerTemplate\", data: outerItem }'></div>";
    ...
    it("Should not be allowed to rewrite templates that embed anonymous templates", function () {
        ...
        ko.setTemplateEngine(
            new dummyTemplateEngine({
                myTemplate:
                    "<div data-bind='template: { data: someData }'>Childprop: [js: childProp]</div>",
            }),
        );
        testNode.innerHTML =
        ...
        ko.setTemplateEngine(
            new dummyTemplateEngine({
                outerTemplate:
                    ...
                    "--><!-- /ko -->",
                innerTemplate:
                    "Inner via inline binding: <span data-bind='text: \"someText\"'></span>",
            }),
        );
        var model = {};
        testNode.innerHTML =
    ...
    it("Should be able to render anonymous templates using virtual containers", function () {
        ...
        testNode.innerHTML =
            "Start <!-- ko template: { data: someData } -->Childprop: [js: childProp]<!-- /ko --> End";
        ko.applyBindings({ someData: { childProp: "abc" } }, testNode);
        expect(testNode).toContainHtml(
            "start <!-- ko template: { data: somedata } -->childprop: abc<!-- /ko -->end",
        );
    });
    ...
    it("Should be able to use anonymous templates that contain first-child comment nodes", function () {
        ...
        ko.setTemplateEngine(new dummyTemplateEngine({}));
        testNode.innerHTML =
            "start <div data-bind='foreach: [1,2]'><span><!-- leading comment -->hello</span></div>";
        ko.applyBindings(null, testNode);
        expect(testNode).toContainHtml(
    ...
    it("Should not throw errors if trying to apply text to a non-rendered node", function () {
        ...
        ko.setTemplateEngine(
            new dummyTemplateEngine({
                myTemplate:
                    "<tr><span data-bind=\"text: 'Some text'\"></span> </tr>", // The whitespace after the closing span is what triggers the strange HTML parsing
            }),
        );
