...
declare module "meteor/mdg:validated-method" {
    export {};
    export type ValidatedMethodName<T> = T extends ValidatedMethod<
        infer TName,
        any
    >
        ? TName
        : never;
    export type ValidatedMethodArg<T> = T extends ValidatedMethod<
        string,
        infer TRun
    >
        ? Argument<TRun>
        : never;
    export type ValidatedMethodReturn<T> = T extends ValidatedMethod<
        string,
        infer TRun
    >
        ? Return<TRun>
    ...
    > extends ValidatedMethodOptionsMixinFields<Argument<TRun>, Return<TRun>> {
        ...
        validate:
            | ((
                  arg: Argument<TRun> extends NoArguments
                      ? any
                      : Argument<TRun>,
              ) => any)
            | null;
    ...
    export interface ValidatedMethodOptionsWithMixins<
        TName extends string,
        TRun extends (...args: any[]) => any,
    ...
    type Return<TFunc> = TFunc extends (...args: any[]) => infer TReturn
        ? TReturn
        : never;
    ...
    > = TOptions extends ValidatedMethodOptions<any, infer TRun>
        ? Return<TRun>
        : never;
    ...
    > {
        constructor(
            options: ValidatedMethodOptionsWithMixins<TName, TRun> &
                ThisType<
                    ValidatedMethodThisBase & {
        ...
        call: Argument<TRun> extends NoArguments
            ? // methods with no argument can be called with () or just a callback
              ((
                  unusedArg: any,
                  callback: (error: Meteor.Error, result: Return<TRun>) => void,
              ) => void) &
                  ((
                      callback: (
