...
function toAsync() {
    ...
    obsStr = <any>(
        Rx.Observable.toAsync((a1: string, a2: number) => a1 + a2.toFixed(0))(
            "",
            1,
        )
    );
    obsStr = <any>(
        Rx.Observable.toAsync(
            (a1: string, a2: number, a3: Date) =>
                a1 + a2.toFixed(0) + a3.toDateString(),
        )("", 1, new Date())
    );
    obsStr = <any>(
        Rx.Observable.toAsync(
            (a1: string, a2: number, a3: Date, a4: boolean) =>
...
function startAsync() {
    const o: Rx.Observable<string> = Rx.Observable.startAsync(
        () => <Rx.IPromise<string>>{},
    );
}
