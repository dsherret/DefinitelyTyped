    ...
    window.addEventListener(
        ...
        function () {
            ...
            req.onreadystatechange = function () {
                ...
                paginate(
                    req.status != 200 &&
                        (req.status != 304 ? false : <any>req.responseText),
                );
            ...
            function paginate(book) {
                var that = this,
                    ...
                    progressBar = <HTMLElement>(
                        document.querySelector("#progressbar > span")
                    ),
                    size;
                ...
                var loopy = function () {
                    ...
                    if (book) {
                        progressBar.style.width =
                            150 -
                            Math.round(progressToBookRatio * progressTotal) +
                            "px";
                        setTimeout(loopy, 1);
                    } else {
                        book = null;
            ...
            ereader.onFlip(function () {
                ...
                for (i = 0; i < 3; i++) {
                    upcoming = (<any>ereader.masterPages[i].dataset)
                        .upcomingPageIndex;

                    if (
                        upcoming !=
                        (<any>ereader.masterPages[i].dataset).pageIndex
                    ) {
