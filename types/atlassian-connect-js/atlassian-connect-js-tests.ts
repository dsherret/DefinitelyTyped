...
AP.request("http://example.com", {
    binaryAttachment: false,
    success: (response: string) => {},
}); // $ExpectType Promise<{ body: string; xhr: XMLHttpRequest }>
...
}); // $ExpectType Promise<{ body: ArrayBuffer; xhr: XMLHttpRequest }>
AP.request({
    url: "http://example.com",
    binaryAttachment: true,
    success: (response: ArrayBuffer) => {},
...
AP.confluence.getMacroData<{ foo: string; bar: number }>(({ foo, bar }) =>
    console.log(foo, bar),
); // $ExpectType void
...
AP.confluence.syncPropertyFromServer("propertyKey", (property) =>
    console.log(property),
); // $ExpectType void
...
AP.jira.getWorkflowConfiguration((workflowConfiguration) =>
