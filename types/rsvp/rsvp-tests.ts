...
function testAll() {
    ...
    RSVP.Promise.all([promise1, promise2, promise3], "my label").then(
        function (array) {
...
function testAllSettled() {
    ...
    RSVP.allSettled([resolved1, resolved2, rejected, pending]).then(
        (states) => {
            ...
            states.forEach((element) => {
                switch (element.state) {
                    case RSVP.State.fulfilled:
                        assertType<RSVP.Resolved<typeof element.value>>(
                            element,
                        );
                        break;

                    case RSVP.State.rejected:
                        assertType<RSVP.Rejected<typeof element.reason>>(
                            element,
                        );
                        break;
