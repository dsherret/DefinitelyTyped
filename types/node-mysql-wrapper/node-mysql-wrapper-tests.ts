...
var db = wrapper2.wrap(
    "mysql://kataras:pass@127.0.0.1/taglub?debug=false&charset=utf8",
);
    ...
    usersDb
        .findSingle({
            ...
            myComments: {
                userId: "=",
                tableRules: {
                    // NEW: SET rules to joined tables too!
        ...
        .then((_user) => {
            // to get this promise use : .promise()
            console.log("\n-------------TEST 2 ------------\n");
            ...
            if (_user.myComments !== undefined) {
                _user.myComments.forEach((_comment) => {
                    ...
                    if (_comment.likes !== undefined) {
                        console.log(
                            " with " + _comment.likes.length + " likes!",
                        );
                    }
                });
            }
