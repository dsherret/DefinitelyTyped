...
myApp.config(
    (
        ...
        const neonRedMap: ng.material.IPalette =
    ...
    (
        ...
        $scope["openBottomSheet"] = () => {
            ...
            options.resolve = {
                ...
                annotatedNativePromise: [
                    "fakeService",
                    (fake) => Promise.resolve(),
                ],
                annotatedAngularPromise: [
                    "fakeService",
                    (fake) => $q.resolve(),
                ],
            };
            ...
            options.controller = [
                "fakeService",
                function TestController(fake: any) {},
            ];
        ...
        $scope["hideBottomSheet"] = $mdBottomSheet.hide.bind(
            $mdBottomSheet,
            "hide",
        );
        $scope["cancelBottomSheet"] = $mdBottomSheet.cancel.bind(
            $mdBottomSheet,
            "cancel",
        );
    },
...
myApp.controller(
    "ColorController",
    ($scope: TestScope, $mdColor: ng.material.IColorService) => {
        const colorExpression: ng.material.IColorExpression = {
            color: "#FFFFFF",
        };
...
myApp.controller(
    ...
    (
        ...
        $scope["alertDialog"] = () => {
            $mdDialog.show(
                $mdDialog.alert().htmlContent("<span>Alert!</span>"),
            );
        };
        $scope["confirmDialog"] = () => {
        ...
        $scope["confirmDialog"] = () => {
            $mdDialog.show(
                $mdDialog.confirm().htmlContent("<span>Confirm!</span>"),
            );
        };
        $scope["promptDialog"] = () => {
        ...
        $scope["promptDialog"] = () => {
            $mdDialog.show(
                $mdDialog.prompt().htmlContent("<span>Prompt!</span>"),
            );
        };
        $scope["promptDialog"] = () => {
            $mdDialog.show(
                $mdDialog.prompt().cancel('Prompt "Cancel" button text'),
            );
        };
        $scope["promptDialog"] = () => {
            $mdDialog.show(
                $mdDialog.prompt().placeholder("Prompt input placeholder text"),
            );
        };
        $scope["promptDialog"] = () => {
        ...
        dialogOptions.controller = [
            "fakeService",
            function TestController(fake) {},
        ];
...
class IconDirective implements ng.IDirective {
    ...
    link($scope: ng.IScope, $elm: ng.IAugmentedJQuery) {
        ...
        this.$mdIcon("work:chair").then((iconEl: Element) =>
            $elm.append(iconEl),
        );
...
myApp.directive(
    "icon-directive",
    ($mdIcon: ng.material.IIcon) => new IconDirective($mdIcon),
);

myApp.controller(
    "MediaController",
    ($scope: TestScope, $mdMedia: ng.material.IMedia) => {
        $scope.$watch(
            () => $mdMedia("lg"),
            (big: boolean) => {
                $scope["bigScreen"] = big;
...
myApp.controller(
    "SidenavController",
    ($scope: TestScope, $mdSidenav: ng.material.ISidenavService) => {
        ...
        $scope["asyncLookup"] = $mdSidenav(componentId, true).then(
            (instance) => {
                instance.toggle();
                instance.open();
                instance.close();
...
myApp.controller(
    ...
    (
        $element: JQuery,
        $scope: TestScope,
        $mdTheming: ng.material.IThemingService,
    ) => {
        $mdTheming($element);

        const PALETTES: ng.material.IConfiguredColorPalette =
            $mdTheming.PALETTES;
        ...
        $scope["setBrowserColors"] = () => {
            ...
            const remove: () => void =
                $mdTheming.setBrowserColor(browserColors);
        };
...
myApp.controller(
    ...
    (
        $scope: TestScope,
        $mdPanel: ng.material.IPanelService,
        $q: ng.IQService,
    ) => {
        $scope["createPanel"] = () => {
...
myApp.controller(
    "StickyController",
    ($scope: TestScope, $mdSticky: ng.material.IStickyService) => {
