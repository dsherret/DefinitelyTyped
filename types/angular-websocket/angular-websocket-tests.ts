...
(
    ...
    scope: ng.IScope,
    provider: ng.websocket.IWebSocketProvider,
) => {
    const socket = provider("wss://localhost");
    const socketWithProtocols = provider("wss://localhost", [
        "protocol-a",
        "protocol-b",
    ]);
    ...
    const socketWithProtocolAndOptions = provider(
        "wss://localhost",
        "protocol",
        {
            scope,
    ...
    socket.onMessageCallbacks.push({
