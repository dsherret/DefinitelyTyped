...
const ctx2d: OffscreenCanvasRenderingContext2D | null =
    offscreenCanvas.getContext("2d");
const ctxBitmaprenderer: ImageBitmapRenderingContext | null =
    offscreenCanvas.getContext("bitmaprenderer");
const ctxWebgl: WebGLRenderingContext | null =
    offscreenCanvas.getContext("webgl");
const ctxWebgl2: WebGL2RenderingContext | null =
...
const blob2: Promise<Blob> = offscreenCanvas.convertToBlob({
    type: "image/jpeg",
});
const blob3: Promise<Blob> = offscreenCanvas.convertToBlob({
    type: "image/jpeg",
    quality: 0.92,
});
...
window.postMessage({ canvas: offscreenCanvas }, "http://example.org:8080", [
    offscreenCanvas,
]);
...
ctxWebgl2!.texImage2D(
    ctxWebgl2!.TEXTURE,
    0,
    ctxWebgl2!.RGB,
    ctxWebgl2!.RGB,
    ctxWebgl2!.UNSIGNED_BYTE,
    offscreenCanvas,
);
