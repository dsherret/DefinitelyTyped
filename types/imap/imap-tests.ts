...
imap.once("ready", function () {
    openInbox(function (err, box) {
        ...
        f.on("message", function (msg: IMAP.ImapMessage, seqno: number) {
            ...
            msg.on(
                "body",
                function (stream: NodeJS.ReadableStream, info: Object) {
                    var buffer = "";
    ...
    imap.search(
        ...
        function (err: Error, results: number[]) {
            ...
            f.on("message", function (msg: IMAP.ImapMessage, seqno: number) {
                ...
                msg.on(
                    "body",
                    function (stream: NodeJS.ReadableStream, info: any) {
                        console.log(prefix + "Body");
                        stream.pipe(
                            fs.createWriteStream("msg-" + seqno + "-body.txt"),
                        );
                    },
                );
                msg.once("attributes", function (attrs: Object) {
