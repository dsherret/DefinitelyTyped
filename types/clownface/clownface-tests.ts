...
function testBlankNode() {
    ...
    const multiBlankContext: AnyPointer<BlankNode[], Dataset> = cf.blankNode([
        "b1",
        "b2",
    ]);
    ...
    const fromMultipleOther: MultiPointer<BlankNode, Dataset> =
        cf.blankNode(multiBlankContext);
...
function testFactory() {
    const defaultContext: AnyPointer<Term | Term[] | undefined, Dataset> =
        clownface({ dataset });

    const namedGraph: AnyPointer<NamedNode, Dataset> = clownface({
        dataset,
        graph,
        term: node,
    });
    const singleFromValue: AnyPointer<Literal, Dataset> = clownface({
        dataset,
        value: "foo",
    });
    ...
    const deriveContextFromOtherGraph: AnyPointer<Literal, Dataset> =
        clownface(literalContext);

    const namedNodeContext: AnyPointer<NamedNode, Dataset> = <any>{};
    const deriveContextFromOtherNamedNodeContext: AnyPointer<
        NamedNode,
        Dataset
    > = clownface(namedNodeContext);
}
...
function testFilter() {
    ...
    const typeGuarded: AnyPointer<Variable, Dataset> =
        anyPointer.filter(onlyVariables);

    function onlyNamedOrBlank(
        ptr: GraphPointer,
    ): ptr is GraphPointer<NamedNode> | GraphPointer<BlankNode> {
        return true;
    }

    const multipleTypeGuarded: AnyPointer<NamedNode | BlankNode, Dataset> =
        anyPointer.filter<NamedNode | BlankNode>(onlyNamedOrBlank);

    const fullSignature = mutliple.filter(
        (
            ptr: GraphPointer<NamedNode>,
            index: number,
            pointers: Array<GraphPointer<NamedNode>>,
        ) => {
            return true;
        },
    );
...
function testHas() {
    const cf: AnyPointer<Term, Dataset> = <any>{};
    let has: AnyPointer<Array<NamedNode | BlankNode>, Dataset> = cf.has(
        predicate,
        "Stuart",
    );
...
function testLiteral() {
    ...
    const cfLiterals: AnyPointer<Literal[], Dataset> = cf.literal([
        "foo",
        "bar",
    ]);
    ...
    const fromMultipleOther: MultiPointer<Literal, Dataset> =
        cf.literal(cfLiterals);
...
function testNamedNode() {
    ...
    const cfNamedMany: AnyPointer<NamedNode[], Dataset> = cf.namedNode([
        "http://example.com/",
        "http://example.org/",
    ]);
    const set: Set<NamedNode> = <any>{};

    const fromOther: AnyPointer<NamedNode, Dataset> =
        cf.namedNode(cfSingleNamed);
    const fromMultipleOther: MultiPointer<NamedNode, Dataset> =
        cf.namedNode(cfNamedMany);
    ...
    const preservedTypeArg: GraphPointer<
        NamedNode<"foo">,
        Dataset
    > = cf.namedNode(foo);
}
...
function testNode() {
    ...
    const cfBlank: AnyPointer<BlankNode, Dataset> = cf.node(null, {
        type: "BlankNode",
    });
    ...
    const literalFromMultipleOther: MultiPointer<Literal, Dataset> =
        cf.node(cfLitMany);
    ...
    const fromArray: MultiPointer<Literal | NamedNode, Dataset> =
        cf.node(pointers);
}
...
function testOutWithLanguage() {
    ...
    let outTerms: Literal[] = cf
        .blankNode()
        .out(node, { language: "en" }).terms;
...
function testToArray() {
    ...
    const mutlipleToArray: Array<AnyPointer<Literal, Dataset>> =
        mutliple.toArray();
}
...
function testMultipleContext() {
    ...
    let singleNamed: AnyPointer<NamedNode, Dataset> = clownface({
        dataset,
    }).namedNode("urn:foo:bar");
    singleNamed = safeCf.namedNode("http://example.com/a");
    singleNamed = clownface({ dataset }).node(node);

    let singleLiteral: AnyPointer<Literal, Dataset> = clownface({
        dataset,
    }).literal("foo");
    singleLiteral = safeCf.literal("a");
    singleLiteral = clownface({ dataset }).node("b");

    const fromSingleArrayBLank: AnyPointer<BlankNode, Dataset> =
        safeCf.blankNode(["b1"]);
    const fromSingleArrayNamed: AnyPointer<NamedNode, Dataset> =
        safeCf.namedNode(["http://example.com/a"]);
    const fromSingleArrayLiteral: AnyPointer<Literal, Dataset> = safeCf.literal(
        ["a"],
    );

    let multipleBlanks: AnyPointer<BlankNode[], Dataset> = safeCf.blankNode([
        "b1",
        "b2",
    ]);
    ...
    let multipleLiterals: AnyPointer<Literal[], Dataset> = safeCf.literal([
        "a",
        "b",
    ]);
    multipleLiterals = clownface({ dataset }).node(["a", 10, false]);

    const multipleMixedTerms: AnyPointer<Term[], Dataset> = clownface({
        dataset,
    }).node(["a", node, null]);
}
...
function testContext() {
    ...
    const ctxGraph: Quad_Graph | undefined =
        fromSingleArrayLiteral._context[0].graph;
...
function addInAddOutRetainsType() {
    ...
    const addOutSingle: AnyPointer<NamedNode, Dataset> = singleNamed.addOut(
        predicate,
        "foo",
    );
    const addOutSingleObjectArray: AnyPointer<NamedNode, Dataset> =
        singleNamed.addOut(predicate, ["foo", "bar"]);
    const addOutSinglePredicateArray: AnyPointer<NamedNode, Dataset> =
        singleNamed.addOut([predicate, predicate]);
    const addOutSingleNoObject: AnyPointer<NamedNode, Dataset> =
        singleNamed.addOut(predicate);
    const addOutSingleWithCallback: AnyPointer<NamedNode, Dataset> =
        singleNamed.addOut(predicate, () => {});
    const addOutSingleWithObjectAndCallback: AnyPointer<NamedNode, Dataset> =
        singleNamed.addOut(predicate, "foo", () => {});

    const addInSingle: AnyPointer<NamedNode, Dataset> = singleNamed.addIn(
        predicate,
        "foo",
    );
    const addInSingleObjectArray: AnyPointer<NamedNode, Dataset> =
        singleNamed.addIn(predicate, ["foo", "bar"]);
    const addInSinglePredicateArray: AnyPointer<NamedNode, Dataset> =
        singleNamed.addIn([predicate, predicate]);
    const addInSingleNoObject: AnyPointer<NamedNode, Dataset> =
        singleNamed.addIn(predicate);
    const addInSingleWithCallback: AnyPointer<NamedNode, Dataset> =
        singleNamed.addIn(predicate, () => {});
    const addInSingleWithObjectAndCallback: AnyPointer<NamedNode, Dataset> =
        singleNamed.addIn(predicate, "foo", () => {});
}
...
function testFilterModule() {
    ...
    let literals: MultiPointer<Literal, Dataset> = pointer.filter(
        taggedLiteral("de"),
    );
...
function testEnvironmentUsage() {
    const env = new Environment([ClownfaceFactory, DatasetFactory]);
