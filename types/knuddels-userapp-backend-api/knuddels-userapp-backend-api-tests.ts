class Server implements App {
    ...
    private readonly appContent: AppContent = AppContent.overlayContent(
        this.htmlFile,
        243,
        266,
    );
    ...
    onUserLeft(user: User) {
        if (this.usersPlaying.get(user.getNick()) === 1) {
            KnuddelsServer.getDefaultBotUser().transferKnuddel(
                user,
                new KnuddelAmount(1),
                {
    ...
    onPrepareShutdown() {
        if (!this.isShuttingDown) {
            ...
            for (const key in this.usersPlaying) {
                ...
                user.getAppContentSessions().forEach(
                    (session: AppContentSession) => {
                        session.remove();
                    },
                );
    ...
    onBeforeKnuddelReceived(knuddelTransfer: KnuddelTransfer) {
        ...
        if (!sender.canSendAppContent(this.appContent)) {
            knuddelTransfer.reject(
                "Sorry, mit diesem Gerät kannst du gerade nicht spielen.",
            );
        } else if (
            sender.isChannelOwner() &&
            knuddelTransfer.getKnuddelAmount().asNumber() !== 1
        ) {
            knuddelTransfer.accept();
        } else if (this.isShuttingDown) {
            knuddelTransfer.reject(
                "Du App nimmt gerade keine neuen Spieler an.",
            );
            ...
            const botNick = KnuddelsServer.getDefaultBotUser()
                ...
                .escapeKCode();
            knuddelTransfer.reject(
                `Du musst genau _°BB>_h1 Knuddel senden|/appknuddel ${botNick}<°°°_...`,
            );
        } else {
            knuddelTransfer.accept();
        }
    ...
    onEventReceived(user: User, key: string, data: string) {
        if (
            key === "selectedEntry" &&
            this.usersPlaying.get(user.getNick()) === 1
        ) {
            ...
            setTimeout(() => {
                ...
                setTimeout(() => {
                    const botNick = KnuddelsServer.getDefaultBotUser()
                        ...
                        .escapeKCode();
                    user.sendPrivateMessage(
                        `Na, Lust auf _°BB>_hnoch eine Runde|/appknuddel ${botNick}<°°°_?`,
                    );
                    user.getAppContentSessions().forEach(
                        (session: AppContentSession) => {
                            session.remove();
