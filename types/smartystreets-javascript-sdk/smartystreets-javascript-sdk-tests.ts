...
const extract_text =
    "Medica Behavioral Health(SM) handles mental health/substance abuse (MH/SA) " +
...
lookup = new SmartyStreetsSDK.usAutocompletePro.Lookup(
    "4646 Scott St, Houston",
);
client
    .send(lookup)
    .then(
...
lookup = new SmartyStreetsSDK.internationalStreet.Lookup(
    "Australia",
    "200 River Terrace",
);
client
...
lookup = new SmartyStreetsSDK.usReverseGeo.Lookup(
    "29.751711638333997",
    "-95.35554678347353",
);
client
...
client =
    SmartyStreetsSDK.core.buildClient.internationalAddressAutocomplete(
        credentials,
    );
lookup = new SmartyStreetsSDK.internationalAddressAutocomplete.Lookup(
    "avenue",
    "CAN",
);
client
