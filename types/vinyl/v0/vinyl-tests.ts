...
describe("File", () => {
    ...
    describe("clone()", () => {
        it("should copy all attributes over with Buffer", (done) => {
            ...
            file2Contents.should.not.equal(
                fileContents,
                "buffer ref should be different",
            );

            let fileUtf8Contents =
                fileContents instanceof Buffer
                    ? fileContents.toString("utf8")
                    : (fileContents as NodeJS.ReadableStream).toString();
            let file2Utf8Contents =
                file2Contents instanceof Buffer
        ...
        it("should copy all attributes over with Stream", (done) => {
            ...
            file2.base.should.equal(file.base);
            file2.path.should.equal(file.path);
            file2.contents.should.equal(
                file.contents,
                "stream ref should be the same",
            );
            done();
        });
    ...
    describe("pipe()", () => {
        it("should write to stream with Buffer", (done) => {
            ...
            stream.on("data", (chunk: any) => {
                should.exist(chunk);
                (chunk instanceof Buffer).should.equal(
                    true,
                    "should write as a buffer",
                );
                chunk
                    .toString("utf8")
                    .should.equal(options.contents.toString("utf8"));
            });
            stream.on("end", () => {
                done();
            });
    ...
    describe("inspect()", () => {
        ...
        it("should return correct format when Buffer and relative path", (done) => {
            ...
            file.inspect().should.equal(
                '<File "test.coffee" <Buffer 74 65 73 74>>',
            );
            done();
        });
        ...
        it("should return correct format when Buffer and only path and no base", (done) => {
            ...
            delete file.base;
            file.inspect().should.equal(
                '<File "/test/test.coffee" <Buffer 74 65 73 74>>',
            );
            done();
        });
        ...
        it("should return correct format when Stream and relative path", (done) => {
            ...
            file.inspect().should.equal(
                '<File "test.coffee" <PassThroughStream>>',
            );
            done();
        });
