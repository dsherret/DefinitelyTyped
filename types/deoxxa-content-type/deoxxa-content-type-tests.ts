...
function new_test(): void {
    ...
    var q = new MediaType("application/json", {
        profile: "http://example.com/schema.json",
    });
    ...
    q.q = 1;
    q.toString() ===
        'application/json;q=1;profile="http://example.com/schema.json"';
}

function mediaCmp_test(): void {
    MediaType.mediaCmp(
        MediaType.parseMedia("text/html"),
        MediaType.parseMedia("text/html"),
    ) === 0;
    MediaType.mediaCmp(
    ...
    MediaType.mediaCmp(
        MediaType.parseMedia("text/html;level=1"),
        MediaType.parseMedia("text/html"),
    ) === -1;
    MediaType.mediaCmp(
