...
const apush = (arr: number[], x: number) => {
    arr.push(x);
    return arr;
};
...
function keepIndexedExample() {
    const xf = t.keepIndexed((i: number, x: string | number) => {
        if (typeof x === "string") return "cool";
    });
...
function wrapExample() {
    const arrayPush = t.wrap((arr: number[], x: number) => {
        arr.push(x);
        return arr;
    });
}
