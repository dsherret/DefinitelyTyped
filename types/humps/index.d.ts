...
export type PascalToSnakeCase<S extends string> =
    S extends `${infer T1}${infer T2}${infer U}`
        ? `${T2 extends Capitalize<T2>
              ? "_"
              : ""}${Lowercase<T1>}${Lowercase<T2>}${CamelToSnakeCase<U>}`
        : S;
...
export function pascalizeKeys<T>(
    str: T,
    optionsOrProcessor?: OptionOrProcessor,
): Pascalized<T>;
...
export function decamelizeKeys<T>(
    str: T,
    optionsOrProcessor?: OptionOrProcessor,
): Decamelized<T>;
...
export function depascalizeKeys<T>(
    str: T,
    optionsOrProcessor?: OptionOrProcessor,
): Depascalized<T>;
