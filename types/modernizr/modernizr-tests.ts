...
$(() => {
    ...
    if (Modernizr.webgl) {
        ...
        const msg =
            "With a different browser you’ll get to see the WebGL experience here: get.webgl.org.";
        document.getElementById("#notice").innerHTML = msg;
    }

    Modernizr.prefixed("boxSizing");
    Modernizr.prefixed("requestAnimationFrame", window);
    const ms = Modernizr.prefixed(
        "matchesSelector",
    ...
    Modernizr.testStyles(
        "#modernizr { width: 9px; color: papayawhip; }",
        (elem, rule) => {
...
Modernizr.testStyles(
    "#modernizr {width: 1px}; #modernizr2 {width: 2px}",
    (elem) => {
