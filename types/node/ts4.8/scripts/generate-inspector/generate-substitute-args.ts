...
const createFieldsForInterface = (
    fields: schema.Parameter[],
    domain: string,
): string[] => {
    return fields
        ? [
...
const createCallbackString = (
    commandName: string,
    returns: schema.Parameter[],
    domain: string,
): string => {
    return hasElements(returns)
        ? `(err: Error | null, params: ${domain}.${capitalize(
              commandName,
          )}ReturnType) => void`
        : `(err: Error | null) => void`;
};
...
const createPostFunctions = (
    command: schema.Command,
    domain: string,
): string[] => {
    const fnName = "post";
    const callbackStr = createCallbackString(
        command.name,
        command.returns || [],
        domain,
    );
 ...
 */
export const generateSubstituteArgs = (
        ...
        .map((item) => {
            ...
            return typePool.length > 0
                ? [
                      ...
                      ...typePool
                          .map((type) =>
                              createTypeDefinition(type, item.domain),
                          )
                          .reduce(flattenArgs(""))
                          .map((line) => `${INDENT}${line}`),
                      "}",
                        ...
                        ? [
                              {
                                  ...
                                  type: `InspectorNotification<${
                                      item.domain
                                  }.${capitalize(event.name)}EventDataType>`,
                              },
                          ]
                        : [],
                }));
