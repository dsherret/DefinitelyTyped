...
controller.addEventListener(
    Leap.LeapEvent.LEAPMOTION_FRAME,
    (event: Leap.LeapEvent) => {
        ...
        if (frame.hands.length > 0) {
            ...
            console.log(
                "Hand pitch: " +
                    Leap.LeapUtil.toDegrees(direction.pitch) +
                    " degrees, " +
                    "roll: " +
                    Leap.LeapUtil.toDegrees(normal.roll) +
                    " degrees, " +
                    "yaw: " +
                    Leap.LeapUtil.toDegrees(direction.yaw) +
                    " degrees\n",
            );
        }
        ...
        for (var i: number = 0; i < gestures.length; i++) {
            ...
            switch (gesture.type) {
                case Leap.Gesture.TYPE_CIRCLE:
                    var circle: Leap.CircleGesture = <Leap.CircleGesture>(
                        gesture
                    );
                    ...
                    if (circle.state != Leap.Gesture.STATE_START) {
                        var previousGesture: Leap.Gesture = controller
                            .frame(1)
                            .gesture(circle.id);
                        if (previousGesture.isValid()) {
                            var previousUpdate: Leap.CircleGesture = <
                                Leap.CircleGesture
                    ...
                    console.log(
                        "Circle id: " +
                            ...
                            circle.radius +
                            ", angle: " +
                            Leap.LeapUtil.toDegrees(sweptAngle) +
                            ", " +
                            clockwiseness,
                    );
                    break;
                ...
                case Leap.Gesture.TYPE_SCREEN_TAP:
                    var screenTap: Leap.ScreenTapGesture = <
                ...
                case Leap.Gesture.TYPE_KEY_TAP:
                    var keyTap: Leap.KeyTapGesture = <Leap.KeyTapGesture>(
                        gesture
                    );
                    console.log(
                        "Key Tap id: " +
