...
export type ArgumentOrMatcher<ArgTypes extends any[]> = {
    [Index in keyof ArgTypes]:
        | ArgTypes[Index]
        | WhenMock<boolean, [ArgTypes[Index]]>;
};
...
export interface WhenMock<T = any, Y extends any[] = any> {
    ...
    expectCalledWith(
        allArgsMatcher: AllArgsMatcher<Y>,
    ): WhenMockWithMatchers<T, Y>;
    expectCalledWith(
        ...matchers: ArgumentOrMatcher<Y>
    ): WhenMockWithMatchers<T, Y>;
    mockReturnValue(value: T): WhenMockWithMatchers<T, Y> & WhenMock<T, Y>;
    mockResolvedValue(
        value: jest.ResolvedValue<T>,
    ): WhenMockWithMatchers<T, Y> & WhenMock<T, Y>;
    mockRejectedValue(
        value: jest.RejectedValue<T>,
    ): WhenMockWithMatchers<T, Y> & WhenMock<T, Y>;
    mockImplementation(
        fn: (...args: Y) => T,
    ): WhenMockWithMatchers<T, Y> & WhenMock<T, Y>;
...
export interface When {
    <T, Y extends any[]>(
        fn: ((...args: Y) => T) | jest.MockInstance<T, Y>,
    ): WhenMock<T, Y>;

    allArgs<Y extends any[]>(
        matcher: (args: Y, equals: jest.MatcherUtils["equals"]) => boolean,
    ): AllArgsMatcher<Y>;
}
