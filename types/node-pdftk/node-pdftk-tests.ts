...
[
    ...
    pdftk
        .allow(["FillIn"])
        .compress()
        .output("./fileoutput.pdf", "./destination/folder"); // $ExpectType Promise<string>
    ...
    PDFTK.input(input)
        ...
        .ignoreWarnings()
        .inputPw("password")
        .burst("page_%02d.pdf"); // $ExpectType Promise<string>
    PDFTK.input(input).flatten().ignoreWarnings().inputPw("password").burst(); // $ExpectType Promise<Buffer>
});

[{ input: [Buffer.from("buffer"), Buffer.from("buffer")] }].forEach(
    (scenario) => {
        const { input } = scenario;
