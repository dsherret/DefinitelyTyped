...
function eachOfIterator<T, K, E>(
    item: T,
    key: K,
    callback: (err: Error) => void,
) {}

function concatIterator<T, R, E>(
    item: T,
    callback: (err: E, res: R[]) => void,
) {}
...
async function testFun() {
    ...
    const q = async.queue<TaskData>(
        (
            task: TaskData,
            callback: (err: Error | null, msg?: string) => void,
        ) => {
