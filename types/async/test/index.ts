...
declare var path: {
    exists: (
        path: string,
        callback?: (err: Error, exists: boolean) => any,
    ) => void;
};
...
function forEachOfIterator(
    item: string,
    key: string,
    forEachOfIteratorCallback: any,
) {
...
promiseString = async.detectLimit(
    ["file1", "file2", "file3"],
    2,
    funcStringCbErrBoolean,
);
...
promiseIterableString = async.sortBy(
    ["file1", "file2", "file3"],
    (file, callback) => {
        fs.stat(file, (err, stats) => {
            callback(err, stats ? stats.mtime : -1);
...
promiseBoolean = async.everyLimit(
    ["file1", "file2", "file3"],
    2,
    funcStringCbErrBoolean,
);
...
let groups: Promise<Record<string, string[]>> = async.groupBy(
    ["file1", "file2", "file3"],
    funcStringCbErrBoolean,
);
async.groupByLimit(["file1", "file2", "file3"], 2, funcStringCbErrBoolean);
groups = async.groupByLimit(
    ["file1", "file2", "file3"],
    2,
    funcStringCbErrBoolean,
);
...
const q = async.queue<any>(
    (task: any, callback: (err?: Error, msg?: string) => void) => {
...
// $ExpectType void
q.push<number, SyntaxError>(
    { name: "bar" },
    (err: SyntaxError | null, result?: number) => {
...
const aq = async.queue<number, number>(
    (level: number, callback: (error?: Error, newLevel?: number) => void) => {
