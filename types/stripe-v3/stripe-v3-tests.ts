...
describe("Stripe object", () => {
    it("should exercise alternative ways to create Stripe object", () => {
        const stripeWithBetaOption: stripe.Stripe = Stripe(
            "pk_test_TYooMQauvdEDq54NiTphI7jx",
            {
                betas: ["beta-feature"],
            },
        ); // This looks deprecated
        const stripeWithLocale: stripe.Stripe = Stripe(
            "pk_test_TYooMQauvdEDq54NiTphI7jx",
            { locale: "zh" },
        );
        const stripeWithAccount: stripe.Stripe = Stripe(
            "pk_test_TYooMQauvdEDq54NiTphI7jx",
            {
                stripeAccount: "acct_24BFMpJ1svR5A89k",
            },
        );
        const stripeWithApiVersion: stripe.Stripe = Stripe(
            "pk_test_TYooMQauvdEDq54NiTphI7jx",
            {
...
describe("Stripe elements", () => {
    ...
    it("should create a cards element", () => {
        ...
        // test 3D secure
        const threeDSecureFrame = <HTMLIFrameElement>(
            document.getElementById("3d-secure-frame")
        );
        const ownerInfo = {
            name: "Jimmy",
            address: {
        ...
        stripe
            .createSource(card, { owner: ownerInfo })
            .then((result) => {
                if (result.error) {
                    // handle error
                ...
                if (
                    !result.source.card ||
                    result.source.card.three_d_secure === "not_supported"
                ) {
                ...
                return stripe.createSource({
                    ...
                    redirect: {
                        return_url:
                            location.origin + "/3d-secure-processing-page",
                    },
                });
            })
            .then((threeDSource) => {
                ...
                if (
                    !threeDSource.source ||
                    threeDSource.source.status === "failed" ||
                    !threeDSource.source.redirect
                ) {
                    // make regular payment...
                    return Promise.resolve(null);
                }
                ...
                threeDSecureFrame.setAttribute(
                    "src",
                    threeDSource.source.redirect.url,
                );
    ...
    it("should create an idealBank element", () => {
        ...
        idealBank.on(
            "change",
            (resp: stripe.elements.ElementChangeResponse) => {
    ...
    it("should use payment request API", () => {
        ...
        paymentRequest.on("token", (ev) => {
            ...
            // post to server...
            Promise.resolve({ ok: true }).then((response) => {
                if (response.ok) {
                    ev.complete("success");
                } else {
                    ev.complete("fail");
