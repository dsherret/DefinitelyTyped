...
export type MethodNames<T> = Values<{
...
export type Super<T> = {
    ...
    [K in MethodNames<T>]: T[K] extends () => infer Return
        ? CallOrApply<T, undefined, Return>
        : T[K] extends (a: infer A) => infer Return
          ? CallOrApply<T, [A], Return>
          : T[K] extends (a: infer A, b: infer B) => infer Return
            ? CallOrApply<T, [A, B], Return>
            : T[K] extends (a: infer A, b: infer B, c: infer C) => infer Return
              ? CallOrApply<T, [A, B, C], Return>
              : T[K] extends (
                      a: infer A,
                      b: infer B,
                      c: infer C,
                ...
                : T[K] extends (...args: any[]) => infer Return
                  ? CallOrApply<T, any[], Return>
                  : never;
};
