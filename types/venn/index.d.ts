...
export interface LayoutParameter {
    lossFunction?:
        | ((sets: { [key: string]: Circle }, overlaps: Overlap[]) => number)
        | undefined;
...
export interface VennDiagram {
    ...
    lossFunction: (
        f: (sets: { [key: string]: Circle }, overlaps: Overlap[]) => number,
    ) => VennDiagram;
    orientationOrder: (
        f: (a: { setid: string }, b: { setid: string }) => number,
    ) => VennDiagram;
}
 ...
 * property).
 */
export function intersectionArea(circles: Array<{}>, stats?: {}): number;
