...
const vennDiagram = venn.VennDiagram().width(100).height(200).duration(300);

vennDiagram.orientationOrder((a: { setid: string }, b: { setid: string }) =>
    a["setid"].localeCompare(b["setid"]),
);

function lossFunction(
    sets: { [key: string]: venn.Circle },
    overlaps: venn.Overlap[],
): number {
    let output = 0;
    for (const area of overlaps) {
        ...
        if (area.sets.length === 1) {
            ...
            if (area.sets.length === 2) {
                ...
                overlapSize = venn.intersectionArea(
                    area.sets.map((i: string) => sets[i]),
                );
            }
        }
        ...
        if (area.size === 0 && overlapSize > 0) {
            output +=
                (overlapSize - area.size) * (overlapSize - area.size) * 100000;
        }
        const weight = area.weight !== undefined ? area.weight : 1;
        output +=
            weight * (overlapSize - area.size) * (overlapSize - area.size);
    }
    return output;
}
