...
function isMoveToCommand(
    cmd: svgParser.Command,
): cmd is svgParser.MoveToCommand {
    return cmd.command === "moveto";
}

function isLineToCommand(
    cmd: svgParser.Command,
): cmd is svgParser.LineToCommand {
    return cmd.command === "lineto";
}

function isHorizontalLineToCommand(
    cmd: svgParser.Command,
): cmd is svgParser.HorizontalLineToCommand {
    return cmd.command === "horizontal lineto";
}

function isVerticalLineToCommand(
    cmd: svgParser.Command,
): cmd is svgParser.VerticalLineToCommand {
    return cmd.command === "vertical lineto";
}

function isClosePathCommand(
    cmd: svgParser.Command,
): cmd is svgParser.ClosePathCommand {
    return cmd.command === "closepath";
}

function isCurveToCommand(
    cmd: svgParser.Command,
): cmd is svgParser.CurveToCommand {
    return cmd.command === "curveto";
}

function isSmoothCurveToCommand(
    cmd: svgParser.Command,
): cmd is svgParser.SmoothCurveToCommand {
    return cmd.command === "smooth curveto";
}

function isQuadraticCurveToCommand(
    cmd: svgParser.Command,
): cmd is svgParser.QuadraticCurveToCommand {
    return cmd.command === "quadratic curveto";
}

function isSmoothQuadraticCurveToCommand(
    cmd: svgParser.Command,
): cmd is svgParser.SmoothQuadraticCurveToCommand {
    return cmd.command === "smooth quadratic curveto";
}

function isEllipticalArcCommand(
    cmd: svgParser.Command,
): cmd is svgParser.EllipticalArcCommand {
...
const cmds: svgParser.Command[] = svgParser.parseSVG(
    "M 10 80 C 40 10, 65 10, 95 80 S 150 150, 180 80",
);
const cmds_made_absolute: svgParser.CommandMadeAbsolute[] =
    svgParser.makeAbsolute(cmds);
