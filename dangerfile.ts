...
const missingProperty =
    /module exports a property named '(.+?)', which is missing/;
...
if (fs.existsSync(suggestionsDir)) {
    ...
    for (const suggestionFile of fs.readdirSync(suggestionsDir)) {
        ...
        const packageName = suggestionFile.slice(
            0,
            suggestionFile.indexOf(".txt"),
        );
        const missingProperties: { [s: string]: string[] } = {};
        for (const s of suggestions) {
            const fileName = s.fileName.slice(
                s.fileName.indexOf("types/" + packageName + "/") +
                    ("types/" + packageName + "/").length,
            );
        ...
        lines.push(
            "## " + packageName + ` ([<kbd>unpkg</kbd>](${topUnpkgURL}))`,
        );
        for (const fileName in missingProperties) {
            if (Object.keys(missingProperties).length > 1) {
                ...
                lines.push(
                    `### ${fileName} ([<kbd>unpkg</kbd>](${unpkgURL}), [<kbd>d.ts</kbd>](${dtsURL}))`,
                );
            }
...
if (
    danger.git.created_files.some((f) => path.basename(f) === ".editorconfig")
) {
    fail(
        "A nested .editorconfig file may not be added to a package on DefinitelyTyped. Please respect the root .editorconfig.",
        danger.git.created_files.find(
            (f) => path.basename(f) === ".editorconfig",
        ),
        1,
    );
}
...
const newPackageJsonFiles = danger.git.created_files.filter((f) =>
...
newPackageJsonFiles.forEach((p) => {
    ...
    if (/"githubUsername":\s*"me"/.test(file)) {
        fail(
            'package.json should have your github username in it, not "me".',
            p,
        );
    }
});
